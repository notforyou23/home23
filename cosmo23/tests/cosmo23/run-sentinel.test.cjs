...
} = require('../../server/lib/run-sentinel');
const { buildStatusContract } = require('../../server/lib/status-contract');
...
test('server /api/stop ACTIVE branch stamps user-stop intent BEFORE stopAll (sub-second race)', () => {
  const src = fsSync.readFileSync(path.resolve(__dirname, '../../server/index.js'), 'utf8');
...
test('index.js wires the sentinel relaunch through the direct-path relauncher, not catalog resolution', async () => {
  const src = await fs.readFile(path.resolve(__dirname, '../../server/index.js'), 'utf8');
