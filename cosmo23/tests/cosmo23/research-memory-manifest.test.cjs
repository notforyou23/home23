...
const { MergeEngine } = require('../../engine/src/merge/merge-engine');
const {
...
async function assertManifestGeneration(runDir, expected) {
  ...
  assert.match(manifest.activeBase.nodes.file, /^memory-nodes\.base-[1-9][0-9]*\.g-[1-9][0-9]*-[a-f0-9-]+\.jsonl\.gz$/);
  assert.match(manifest.activeBase.edges.file, /^memory-edges\.base-[1-9][0-9]*\.g-[1-9][0-9]*-[a-f0-9-]+\.jsonl\.gz$/);
