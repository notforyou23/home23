...
async function runCancellationScenario(sampler) {
  const { projectPinnedQuery } = require('../../lib/pinned-query-projection.js');
...
async function runQueryScenario(scenario, sampler) {
  const { QueryEngine } = require('../../lib/query-engine.js');
  const {
    createQueryOperationExecutor,
  } = require('../../server/lib/query-operation-worker.js');
