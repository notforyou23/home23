...
const { auditArtifactLoop } = require('../../engine/src/artifacts/artifact-audit');
const { migrateExistingOutputs } = require('../../engine/src/artifacts/artifact-migration');
const { ArtifactLifecycleManager } = require('../../engine/src/artifacts/artifact-lifecycle');
const { verifyArtifactLoop } = require('../../engine/src/artifacts/artifact-loop-verifier');
const { TaskStateQueue } = require('../../engine/src/cluster/task-state-queue');
const FilesystemStateStore = require('../../engine/src/cluster/backends/filesystem-state-store');
const { AgentExecutor } = require('../../engine/src/agents/agent-executor');
const { Capabilities } = require('../../engine/src/core/capabilities');
...
test('artifact context is injected into mission description only once', () => {
  ...
  const baseAgent = {
    mission,
    _uploadedArtifacts: [],
    getMissionWithArtifactContext: require('../../engine/src/agents/base-agent').BaseAgent.prototype.getMissionWithArtifactContext
  };
