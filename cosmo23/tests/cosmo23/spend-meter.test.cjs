...
} = require('../../engine/src/core/spend-meter');
...
const AnthropicClient = require('../../engine/src/core/anthropic-client');
...
test('launch payload spend keys round-trip through serialize -> metadata.json -> replay (source contract)', () => {
  ...
  const generatorSource = readSource('../../launcher/config-generator.js');
...
test('the seven leaf metering sites exist exactly once each — no double counting', () => {
  const gpt5Source = readSource('../../engine/src/core/gpt5-client.js');
  const ccSource = readSource('../../engine/src/core/chat-completions-client.js');
  const anthropicSource = readSource('../../engine/src/core/anthropic-client.js');
  const unifiedSource = readSource('../../engine/src/core/unified-client.js');
...
test('orchestrator binds, resumes, exposes and flushes the meter exactly once each', () => {
  const orchestratorSource = readSource('../../engine/src/core/orchestrator.js');
