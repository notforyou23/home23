...
- Every phase records a concrete `expectedOutput`; when Launch omitted the field, a file explicitly named in the phase mission becomes the contract, otherwise Cosmo assigns a unique phase receipt. It closes only when that exact phase-provenanced receipt is still on disk, matches the bytes written by the phase, and contains finished work; progress notes, empty findings, another phase's file, changed receipts, and temporary dumps cannot close it.
