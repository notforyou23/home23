 ...
 * 2. ~/.cosmo2.3/config.json security.encryption_key
...
function getEncryptionKey() {
  ...
  try {
    ...
    const configPath = process.env.COSMO23_CONFIG_PATH ||
      process.env.EVOBREW_CONFIG_PATH ||
      path.join(os.homedir(), '.cosmo2.3', 'config.json');
...
function deriveMachineKey() {
  ...
  const salt = 'cosmo-2-3-config-salt'; // Fixed salt for determinism
