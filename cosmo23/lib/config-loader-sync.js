/**
 * COSMO 2.3 - Synchronous Configuration Loader
 * 
...
function deriveMachineKey() {
  ...
  const salt = 'cosmo-2-3-config-salt';
...
const DEFAULT_CONFIG = {
  ...
  providers: {
    ...
    anthropic: { enabled: false, oauth: true },
...
function loadConfigurationSync(options = {}) {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      log('[CONFIG] ✓ Loaded from ~/.cosmo2.3/config.json');
  ...
  log('[CONFIG] Configure providers to create ~/.cosmo2.3/config.json');
...
function envToConfig() {
  ...
  if (process.env.COSMO23_PORT || process.env.PORT) {
    config.server.http_port = parseInt(process.env.COSMO23_PORT || process.env.PORT, 10);
  }
  if (process.env.COSMO23_HTTPS_PORT || process.env.HTTPS_PORT) {
    config.server.https_port = parseInt(process.env.COSMO23_HTTPS_PORT || process.env.HTTPS_PORT, 10);
  }
...
function applyConfigToEnv(config) {
  // Server
  if (config.server) {
    if (!process.env.COSMO23_PORT) {
      process.env.COSMO23_PORT = String(config.server.http_port || 43110);
    }
    if (!process.env.PORT) {
      process.env.PORT = String(config.server.http_port || 43110);
    }
    if (!process.env.COSMO23_HTTPS_PORT) {
      process.env.COSMO23_HTTPS_PORT = String(config.server.https_port || 43111);
    }
    if (!process.env.HTTPS_PORT) {
      process.env.HTTPS_PORT = String(config.server.https_port || 43111);
    }
  }
