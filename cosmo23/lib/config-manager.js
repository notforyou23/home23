/**
 * COSMO 2.3 - Configuration Manager
 * 
 * Manages global configuration stored in ~/.cosmo2.3/
...
/**
 * Get the path to the main config file (~/.cosmo2.3/config.json)
 * @returns {string}
 */
...
/**
 * Get the path to the database file (~/.cosmo2.3/database.db)
 * @returns {string}
 */
...
/**
 * Get the path to the logs directory (~/.cosmo2.3/logs/)
 * @returns {string}
 */
function getLogsDir() {
...
/**
 * Get the path to the SSL directory (~/.cosmo2.3/ssl/)
 * @returns {string}
 */
function getSslDir() {
 ...
 * Creates ~/.cosmo2.3/ and subdirectories if they don't exist.
...
/**
 * Load configuration from ~/.cosmo2.3/config.json
...
async function loadConfig() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error(`Config file not found: ${configPath}. Save provider setup first.`);
  }
...
/**
 * Save configuration to ~/.cosmo2.3/config.json
...
async function migrateFromEnv(envPath) {
  ...
  // Server settings
  if (env.COSMO23_PORT || env.PORT) {
    config.server.http_port = parseInt(env.COSMO23_PORT || env.PORT, 10);
  }
  if (env.COSMO23_HTTPS_PORT || env.HTTPS_PORT) {
    config.server.https_port = parseInt(env.COSMO23_HTTPS_PORT || env.HTTPS_PORT, 10);
  }
...
/**
 * Migrate database from old location to ~/.cosmo2.3/database.db
...
function configToEnv(config) {
  ...
  // Server
  if (config.server) {
    env.COSMO23_PORT = String(config.server.http_port || 43110);
    env.COSMO23_HTTPS_PORT = String(config.server.https_port || 43111);
    env.PORT = String(config.server.http_port || 43110);
    env.HTTPS_PORT = String(config.server.https_port || 43111);
  }
