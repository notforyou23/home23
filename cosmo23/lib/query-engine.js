...
class QueryEngine {
  constructor(runtimeDir, openaiKey) {
    ...
    this.coordinatorIndexer = this.openai
      ? new CoordinatorIndexer(this.coordinatorDir, this.openai, this.embeddingConfig)
      : null;
...
`.trim();
  ...
  async getEmbedding(text) {
    if (!this.openai || this.embeddingConfig?.provider !== 'openai') {
