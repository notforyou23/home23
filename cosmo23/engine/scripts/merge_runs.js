...
const CONFIG = {
  runsDir: process.env.COSMO_RUNS_PATH || path.join(__dirname, '..', 'runs'),
...
class MergeEngine {
  ...
  async copyWorkArtifacts(loadedStates, outputDir) {
    ...
    for (const loaded of loadedStates.filter(s => s.valid)) {
      ...
      // Use actual path from loadedState if available (Hub mode), otherwise use CONFIG.runsDir (CLI mode)
      const sourcePath = loaded.path || path.join(CONFIG.runsDir, sourceName);
