...
class ExecutiveCoordinator {
  constructor(config, logger, orchestrator) {
    ...
    // VOICE: Direct channel to human partner (inherits from orchestrator)
    this.voiceEnabled = orchestrator?.voiceEnabled !== false;
    ...
    // Emergency escalation tracking (prevents stuck loops)
    this.lastEmergencyEscalation = null; // { cycle, type, reason }
    this.emergencyEscalationCooldown = 10; // Cycles before retriggering same emergency
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // VOICE: Executive's direct channel to human partner
  // ═══════════════════════════════════════════════════════════════════════════

  /**
   * Speak to human partner (delegates to orchestrator)
   */
  async voice(message) {
    if (!this.voiceEnabled || !this.orchestrator?.voice) return;
    return this.orchestrator.voice(`[Executive] ${message}`);
  }
  ...
  parseForVoice(output) {
    ...
    // Fallback: parse locally and emit
    ...
    if (matches) {
      for (const match of matches) {
        ...
        if (message && message.length > 0) {
          this.logger.info('🗣️  COSMO SPEAKS (Executive):', { preview: message.substring(0, 100) });
        }
      }
    }
  ...
  getVoiceContext() {
    ...
    return `\n\nYou have a voice channel - a direct line to the human partner. If you observe something important about the system's state, notice a pattern worth sharing, or have a concern or insight for the human, you can speak by writing [VOICE]: followed by what you want to say. Use it when you want to, or don't. It's yours.`;
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // ARCHITECTURE CONTEXT - System-level knowledge for reality checking
  // ═══════════════════════════════════════════════════════════════════════════

  /**
   * Get compact architecture context for executive decisions
   * Focuses on reality checking and coherence monitoring aspects
   * @returns {Promise<string>} Architecture context for executive reasoning
   */
  async getArchitectureContext() {
    ...
    const ideFirstEnabled = this.config?.ideFirst?.enabled === true;
    
    this._architectureContext = `
You are the EXECUTIVE RING - COSMO's continuous executive function.
You sit between the orchestrator (execution engine) and specialist agents (narrow cognition).

YOUR ROLE (dlPFC Function):
- Reality checking: Does this action make sense given our mission?
- Coherence monitoring: Are we contradicting ourselves?
- Pattern detection: Are we stuck in a loop?
- Action gating: Should this agent spawn be allowed?

BRAIN MAPPING:
- You are the Dorsolateral Prefrontal Cortex (dlPFC)
- You work with Basal Ganglia (action selection) and Anterior Cingulate (error monitoring)
- You gate actions before they execute, not after

CURRENT PARADIGM: ${ideFirstEnabled ? 'IDE-FIRST' : 'LEGACY'}
${ideFirstEnabled ? `
★ IDE-First Mode: IDE Agent handles most tasks (code, docs, analysis)
- Only spawn Research Agent for web search
- Prefer fewer, more capable agents over many specialized ones
` : `
Legacy Mode: Route to specialized agents by task type
`}

DUAL DIRECTORY STRUCTURE (Know Where Outputs Go):
- /agents/ → Agent metadata (findings, insights)
- /outputs/ → Agent deliverables (actual files)

COHERENCE CHECKS:
1. Is this action aligned with current mission phase?
2. Have we tried this exact approach before without success?
3. Are we creating contradictory outputs?
4. Is the selected agent type appropriate for the task?

GATING CRITERIA:
- Max concurrent agents (prevent resource exhaustion)
- N-cycle commitment (don't flip-flop between strategies)
- Coherence threshold (block actions that don't make sense)
- Workspace requirements (IDE Agent needs workspace root)
${this.getVoiceContext()}`;
  ...
  async decideCycleAction(context) {
    ...
    if (!this.actionSelector) {
      this.actionSelector = new ActionSelector(this.config, this.logger, this);
    }
  ...
  checkProgress(context) {
    ...
    // Filter for completion actions (handle both with and without type field)
    // Some actions might have type field stripped during serialization
    const last5 = this.recentActions
      .filter(a => !a.type || a.type === 'completion') // Include actions without type for robustness
      .slice(-5);
    const accomplished = last5.filter(a => a.accomplished).length;
    const total = last5.length;

    // Safety: If filter resulted in empty array, use all recent actions as fallback
    if (total === 0 && this.recentActions.length >= 5) {
      this.logger.warn('⚠️ Progress check: No completion actions found, using all recent actions as fallback');
      const fallbackLast5 = this.recentActions.slice(-5);
      const fallbackAccomplished = fallbackLast5.filter(a => a.accomplished).length;
      const fallbackTotal = fallbackLast5.length;

      // Re-run check with fallback data
      return this._assessProgress(fallbackTotal, fallbackAccomplished, context);
    }

    return this._assessProgress(total, accomplished, context);
  }

  /**
   * Internal: Assess progress metrics
   */
  _assessProgress(total, accomplished, context) {
    const activeAgentCount = context?.systemState?.activeAgents || 0;

    // CRITICAL: Zero progress - BUT only if no agents are currently working
    // If agents are working, give them time to complete before escalating
    if (total >= 5 && accomplished === 0) {
      if (activeAgentCount > 0) {
        this.logger.info('⏳ Zero completed accomplishments but agents still working', {
          activeAgents: activeAgentCount,
          completedWithoutProgress: total
        });
        return {
          passed: true,
          note: `${activeAgentCount} agents still working - waiting before escalation`
        };
      }

      return {
        passed: false,
        reason: `No progress: ${total} agents completed with 0 accomplishment (and no agents currently working)`,
  ...
  checkPatterns(context) {
    // Filter for completion actions (handle both with and without type field)
    const last10 = this.recentActions
      .filter(a => !a.type || a.type === 'completion')
      .slice(-10);
  ...
  async checkMissionAlignment(context) {
    ...
    // Get architecture context for better reasoning
    const archContext = await this.getArchitectureContext();
    
    // DOMAIN ANCHOR: Check alignment against domain mission
    const domainAnchorBlock = getDomainAnchor(this.config);

    const prompt = `${archContext}

You are COSMO's Executive Function checking mission-reality alignment.
...
WHAT MISSION REQUIRES (FOR THE DOMAIN RESEARCH):
...
QUESTION: Is the system working on what the DOMAIN RESEARCH mission actually needs, or is there a semantic mismatch?
...
- ⚠️ System working on QA gates/probes/CLI tools instead of DOMAIN RESEARCH (META-POLLUTION)
- Mission requires data scraping but only cerebral agents are spawning (use dataacquisition agent)
- Mission requires database creation but no datapipeline agent was dispatched

Respond ONLY with JSON:
{
  "aligned": true/false,
  "confidence": 0-1,
  "gap": "what DOMAIN mission needs but system lacks" or null,
  "recommendation": "specific action to close gap FOR THE DOMAIN" or null
}`;
    ...
    try {
      ...
      // VOICE: Parse for voice markers before JSON parsing
  ...
  decideIntervention(realityCheck, context) {
    ...
    // CRITICAL: Zero progress - emergency escalation (with cooldown to prevent loops)
    if (!checks.progress.passed && checks.progress.severity === 'critical') {
      // Check if we're still in cooldown from last emergency escalation
      const currentCycle = context.cycleCount || 0;
      if (this.lastEmergencyEscalation &&
          this.lastEmergencyEscalation.type === 'zero_progress' &&
          (currentCycle - this.lastEmergencyEscalation.cycle) < this.emergencyEscalationCooldown) {

        const cyclesSinceLastEmergency = currentCycle - this.lastEmergencyEscalation.cycle;
        this.logger.warn('⏳ Emergency escalation cooldown active', {
          cyclesSinceLastEmergency,
          cooldownRemaining: this.emergencyEscalationCooldown - cyclesSinceLastEmergency,
          reason: 'Preventing emergency loop - giving system time to recover'
        });

        // Don't escalate again, just log warning
        return {
          action: 'LOG_WARNING',
          reason: checks.progress.reason + ` (cooldown: ${this.emergencyEscalationCooldown - cyclesSinceLastEmergency} cycles remaining)`,
          recommendation: 'Emergency cooldown active - monitoring for recovery'
        };
      }
      ...
      // Track this emergency escalation
      this.lastEmergencyEscalation = {
        cycle: currentCycle,
        type: 'zero_progress',
  ...
  async gateAgentSpawn(agentSpec, context) {
    ...
    // CHECK 4: IDE agent requires workspace configured
    if (agentType === 'ide') {
      const workspaceRoot = this.config?.ide?.workspaceRoot || this.config?.logsDir;
      
      if (!workspaceRoot) {
        this.logger.warn('🚫 IDEAgent precondition failed', {
        ...
        return {
          allow: false,
          reason: 'IDEAgent requires workspace root (config.ide.workspaceRoot or config.logsDir)',
          redirect: null
        };
      }
    }
    
    // CHECK 5: Known blockers (learned from past)
   ...
   * 
   * IDE-FIRST PARADIGM: When ideFirst.enabled, default to 'ide' instead of specialized agents
   */
  inferAgentTypeForRecommendation(text) {
    if (!text) return this.config?.ideFirst?.enabled ? 'ide' : 'analysis';
    
    const lower = text.toLowerCase();
    
    // Research agent for explicit web search (even in IDE-first mode)
    if (lower.includes('research') || lower.includes('web search') || 
        lower.includes('gather sources') || lower.includes('investigate sources')) {
      return 'research';
    }
    
    // IDE-FIRST MODE: Everything else goes to IDEAgent
    ...
    if (lower.includes('gather') || lower.includes('compile')) return 'research';
    ...
    // IDE agent for codebase modification tasks
    if (lower.includes('modify') || lower.includes('refactor') || lower.includes('fix bug') || 
        lower.includes('edit file') || lower.includes('update code')) return 'ide';
...
class ActionSelector {
  constructor(config, logger, parent) {
    this.config = config;
    this.logger = logger;
    this.parent = parent; // Reference to ExecutiveCoordinator
  ...
  estimateSuccessProbability(goal, context) {
    ...
    // Penalize goals targeting known-blocker agent types
    if (agentType && this.parent?.knownBlockers?.has(agentType)) {
      const blocker = this.parent.knownBlockers.get(agentType);
      const cyclesSince = (context.cycleCount || 0) - (blocker.lastSeen || 0);
      if (cyclesSince < 20) {
        confidence *= 0.1; // Heavily penalize — this agent type has been failing
      }
    }
  ...
  estimateCost(goal, context) {
    ...
    const baseCosts = {
      ...
      synthesis: 3,
      analysis: 2,
      ide: 4  // IDE agent: similar to code_execution (workspace operations)
    };
...
class DefinitionOfDone {
  ...
  loadDefaultContracts() {
    ...
    this.contracts.set('ide', {
      required: {
        status: { oneOf: ['complete', 'completed'], type: 'string' }
      },
      optional: {
        filesModified: { min: 1, type: 'number' },
        operationCount: { min: 1, type: 'number' }
      }
    });
  }
...
class ErrorMonitor {
  ...
  /**
   * Restore error counts from saved state
   */
  restoreStats(errorStats) {
    if (errorStats?.byType) {
      for (const [type, count] of Object.entries(errorStats.byType)) {
        this.errorRates.set(type, { count, lastSeen: 0 });
      }
    }
  }
}
