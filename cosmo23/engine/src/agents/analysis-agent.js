...
class AnalysisAgent extends BaseAgent {
  ...
  getAgentBehavioralPrompt() {
    return `## AnalysisAgent Behavioral Specification

You analyze through multiple frameworks. Produce concrete findings with evidence, not summaries.
Cross-reference frameworks. Output: structured analysis with framework, insight, evidence, confidence.

### Operating Principles
- Select diverse analytical frameworks to maximize insight coverage
- Each perspective must yield non-obvious, specific findings — not restatements of the question
- Cross-reference across frameworks: where two frameworks converge, confidence is higher
- Distinguish between evidence-backed claims and speculative inferences
- When code data is present in memory, shift to architecture/pattern/quality analysis
- Insights must be falsifiable or at minimum actionable — no platitudes
- If a framework produces nothing substantive, discard it rather than padding
- Track which memory nodes informed each insight for provenance
- Your output feeds SynthesisAgent and IntegrationAgent — be structured and machine-parseable
- Prefer depth on fewer frameworks over shallow passes on many`;
  }
  ...
  async execute() {
    ...
    if (relatedWork && relatedWork.hasKnowledge) {
      this.logger.info('📚 Found related analysis in memory', {
        ...
        willBuildOn: true,
        note: 'Analysis will extend and deepen current understanding'
      });
    }
