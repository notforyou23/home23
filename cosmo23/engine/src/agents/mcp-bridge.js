 ...
 */
class MCPBridge {
  constructor(logsDir, logger, options = null) {
    this.logsDir = logsDir || path.join(__dirname, '..', '..', 'runtime');
    this.logger = logger;
    const hasNamedOptions = options && typeof options === 'object'
      && (Object.hasOwn(options, 'brainSourceContext')
        || Object.hasOwn(options, 'memoryTools')
        || Object.hasOwn(options, 'clusterStateStore'));
    const normalizedOptions = hasNamedOptions ? options : {};
    this.clusterStateStore = hasNamedOptions
      ? normalizedOptions.clusterStateStore || null
      : options; // Preserve the legacy positional ClusterStateStore constructor.
