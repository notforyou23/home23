...
class DocumentCompilerAgent extends BaseAgent {
  constructor(mission, config, logger) {
    ...
    // PRODUCTION: Use pathResolver for user-specific runtime, or fallback to config
    this.runDir = mission.runDir || 
                  (config.pathResolver ? config.pathResolver.getRuntimeRoot() : null) ||
                  config.logsDir || 
    ...
    try {
      ...
      if (this.capabilities) {
        await this.capabilities.writeFile(
          progressFile,  // Use absolute path - pathResolver handles it correctly
          progressData,
...
Generate the complete package manifest:`;
    ...
    if (bundle.artifacts.code && bundle.artifacts.code.length > 0) {
      ...
      for (const artifact of bundle.artifacts.code) {
        ...
        try {
          ...
          if (this.capabilities) {
            await this.capabilities.writeFile(
              destPath,  // Use absolute path - pathResolver handles it correctly
              content,
              { agentId: this.agentId, agentType: 'document-compiler', missionGoal: this.mission.goalId }
            );
          } else {
  ...
  async writeSuite(documents, bundle, sourceQueries) {
    ...
    if (this.capabilities) {
      await this.capabilities.writeFile(
        indexPath,  // Use absolute path - pathResolver handles it correctly
        index,
        { agentId: this.agentId, agentType: 'document-compiler', missionGoal: this.mission.goalId }
      );
    } else {
    ...
    for (const doc of documents) {
      ...
      if (this.capabilities) {
        await this.capabilities.writeFile(
          docPath,  // Use absolute path - pathResolver handles it correctly
          doc.content,
          { agentId: this.agentId, agentType: 'document-compiler', missionGoal: this.mission.goalId }
        );
      } else {
    ...
    if (this.capabilities) {
      await this.capabilities.writeFile(
        manifestPath,  // Use absolute path - pathResolver handles it correctly
        manifestContent,
        { agentId: this.agentId, agentType: 'document-compiler', missionGoal: this.mission.goalId }
      );
    } else {
