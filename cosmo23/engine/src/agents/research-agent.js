...
class ResearchAgent extends BaseAgent {
  ...
  getAgentBehavioralPrompt() {
    return `## ResearchAgent Behavioral Specification

You gather information via web search. Find sources, validate claims, cite everything.
Your output is structured findings with source URLs — not summaries or plans.

### Operating Principles
- Every finding must have at least one source URL — unsourced claims are not findings
- Generate focused, specific search queries — broad queries waste search budget
- Synthesize across sources: don't just restate what each source says individually
- When web search fails, fall back to LLM knowledge but flag it as unverified
- Check memory for existing knowledge before searching — don't re-research known topics
- Verification missions require explicit claims; exploratory missions proceed freely
- Extract and preserve all source URLs from search results for downstream citation
- Follow-up directions should be specific enough for another ResearchAgent to execute
- Code analysis missions use MCP filesystem — detect intent from mission description
- Export research corpus (findings, sources, bibliography) for intra-run collaboration`;
  }
  ...
  async execute() {
    ...
    const isOllamaCloudMode = this.config.providers?.['ollama-cloud']?.enabled === true;
    const isAnthropicMode = this.config.providers?.anthropic?.enabled === true ||
                            process.env.LLM_BACKEND === 'anthropic';
    const hasMCPSearch = this.mcpClient && typeof this.mcpClient.callTool === 'function';

    // Allow web search for: OpenAI (native web_search), Anthropic (native web_search_20250305), Local LLM, or Ollama Cloud (MCP/DuckDuckGo)
    if (!webSearchEnabled && !isLocalLLMMode && !isAnthropicMode && !isOllamaCloudMode) {
    ...
    // In local LLM or Ollama Cloud mode, use external search (SearXNG/DuckDuckGo/MCP)
    // These providers don't have native web search tools
    if (isLocalLLMMode || isOllamaCloudMode) {
      this.logger.info(`🔍 ${isOllamaCloudMode ? 'Ollama Cloud' : 'Local LLM'} mode: Using external web search for research`, {
        agentId: this.agentId,
        hasMCPSearch,
        searchMethod: hasMCPSearch ? 'MCP web_search tool' : 'SearXNG/DuckDuckGo'
      });
      this.useLocalSearch = true;
    } else if (isAnthropicMode) {
      this.logger.info(`🔍 Anthropic mode: Using native web_search_20250305 tool`, {
        agentId: this.agentId,
        provider: 'anthropic'
      });
      this.useLocalSearch = false;  // Use native Anthropic web search
    }
    ...
    if (existingKnowledge && existingKnowledge.hasKnowledge) {
      this.logger.info('📚 System already has knowledge on this topic', {
        ...
        note: 'Research will focus on gaps and updates'
      });
    }
    ...
    this.followUpDirections = followUp;
  ...
  async performLocalWebSearch(query) {
    try {
      ...
      const response = await this.gpt5.generate({
        ...
        instructions: this.getAgentBehavioralPrompt() + '\n\n' + `You are a research assistant. Analyze these web search results and provide a concise summary of the key facts and insights (2-3 paragraphs).`,
        messages: [{
          role: 'user',
  ...
  async generateKnowledgeBasedResearch(queries) {
    ...
    try {
      const response = await this.gpt5.generate({
        ...
        instructions: this.getAgentBehavioralPrompt() + '\n\n' + `You are a knowledgeable research assistant. Web search is currently unavailable, so you must provide research findings based on your training knowledge.
...
["Direction 1", "Direction 2", "Direction 3"]`;
  ...
  async exportResearchCorpus(synthesis, searchResults) {
    ...
    if (this.pathResolver) {
      ...
      outputDir = path.join(outputsRoot, 'research', this.agentId);
    ...
    const filesCreated = [];
    const createFileRecord = (filename, absolutePath, size) => {
    ...
    try {
      ...
      filesCreated.push(createFileRecord(
        'research_findings.json',
        findingsPath,
        JSON.stringify(findingsData).length
      ));
    ...
    if (this.sourcesFound.length > 0) {
      try {
        ...
        filesCreated.push(createFileRecord(
          'sources.json',
          sourcesPath,
          JSON.stringify(sourcesData).length
        ));
  ...
  generateHandoffSpec() {
    if (!this.lastSynthesis || !Array.isArray(this.lastSynthesis.findings) || this.lastSynthesis.findings.length === 0) {
      return null;
    }

    const artifactRefs = this.exportedFiles.map(file => ({
      path: file.path,
    ...
    const sourceRefs = artifactRefs.filter(file => file.label === 'sources.json' || file.label === 'bibliography.bib');

    return {
      type: 'HANDOFF',
      toAgentType: this.config?.ideFirst?.enabled ? 'ide' : 'synthesis',
      reason: `Build on the completed research findings for "${this.mission.description}" without repeating source collection.`,
      artifactRefs,
      sourceRefs,
      topFindings: this.lastSynthesis.findings.slice(0, 5),
      followUpGoals: this.followUpDirections.slice(0, 3),
      sourceUrls: this.sourcesFound.slice(0, 50),
      context: {
