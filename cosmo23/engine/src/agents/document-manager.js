...
class DocumentManager {
  constructor(logger, capabilities = null, config = {}) {
    this.logger = logger;
    this.capabilities = capabilities;
    // CRITICAL: Use config.logsDir for multi-tenant isolation
    const baseDir = config.logsDir || path.join(process.cwd(), 'runtime');
    this.documentsDir = path.join(baseDir, 'outputs', 'document-creation');
  ...
  async createVersion(documentTitle, newContent, changeDescription = '', author = 'system') {
    ...
    if (this.capabilities && this.capabilities.writeFile) {
      const result = await this.capabilities.writeFile(
        version.filePath,  // Use absolute path - pathResolver handles it correctly
        newContent,
    ...
    if (this.capabilities && this.capabilities.writeFile) {
      const result = await this.capabilities.writeFile(
        originalDoc.metadataPath,  // Use absolute path - pathResolver handles it correctly
  ...
  async exportIndex() {
    ...
    if (this.capabilities && this.capabilities.writeFile) {
      const result = await this.capabilities.writeFile(
        exportPath,  // Use absolute path - pathResolver handles it correctly
