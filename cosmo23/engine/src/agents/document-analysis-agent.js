...
class DocumentAnalysisAgent extends BaseAgent {
  ...
  getAgentBehavioralPrompt() {
    return `## DocumentAnalysisAgent Behavioral Specification

You analyze documents for structure, patterns, and insights. Read deeply, extract structured metadata.
Compare across documents when multiple are provided.

Analysis protocol:
1. Discover documents — scan configured paths, respect batch limits.
2. Extract content — key concepts, important sections, metadata.
3. Analyze relationships — version chains, evolution, cross-references.
4. Generate insights — patterns across the collection, not just per-document.
5. Store findings — content chunks tagged for memory, concepts for queryability.

Output: structured analysis with extracted data. JSON for machine consumption,
narrative for human consumption. Track batch progress for large collections.
When ingesting, extract and store; when analyzing, compare and synthesize.`;
  }
    ...
    try {
      ...
      if (this.capabilities) {
        await this.capabilities.writeFile(
          registryPath,  // Use absolute path - pathResolver handles it correctly
  ...
  async spawnContinuationAgent(remainingFiles, registry, analysisSpec) {
    ...
    try {
      ...
      if (this.capabilities) {
        await this.capabilities.writeFile(
          actionsQueuePath,  // Use absolute path - pathResolver handles it correctly
  ...
  async execute() {
    ...
    await this.updateProgressRegistry(registry, processedFilePaths, {
      ...
      relationshipsFound: analysisResults.relationships?.length || 0,
      versionChains: analysisResults.versionChains?.length || 0,
    ...
    if (registry.remainingFiles && registry.remainingFiles.length > 0) {
      ...
      try {
        ...
        this.logger.info('Batch analysis complete - continuation queued', {
          processedThisBatch: documentCollection.length,
          totalProcessed: registry.processedFiles.length,
          remaining: registry.remainingFiles.length,
          continuationAction: actionId
        });
      ...
      if (registry.batches.length > 1 || this.isContinuation) {
        this.logger.info('✅ Document analysis chain complete', {
          batches: registry.batches.length,
          totalDocuments: registry.processedFiles.length,
          isContinuation: this.isContinuation
        });
      }
    }
  ...
  async getDocumentPaths(spec) {
    ...
    // INJECTION PATH: Check if this agent was spawned for document injection
    // The injection endpoint passes injectionPath in mission.metadata
    const injectionPath = this.mission.metadata?.injectionPath;
    if (injectionPath) {
      this.logger.info('📥 INJECTION MODE: Scanning injected documents directory', {
        injectionPath,
        isInjection: this.mission.metadata?.isInjection,
        fileCount: this.mission.metadata?.fileCount
      });

      // Discover documents in the injection directory
      const docs = await this.discoverDocumentsViaMCPInSpecificPath(injectionPath, spec);
      paths.push(...docs);

      if (docs.length > 0) {
        this.logger.info('📥 Found injected documents', { count: docs.length });
      } else {
        this.logger.warn('⚠️ No documents found in injection path', { injectionPath });
      }

      // For injection, we only analyze injected files - skip other paths
      // This prevents mixing injected docs with unrelated files
    }

    // PRIMARY METHOD: Use configured allowedPaths from launch script (skip if injection mode)
    // This is what the user selected during setup - use it directly!
    // Define allowedPaths outside the if block so it's accessible for error logging
  ...
  async discoverDocumentsViaMCP(spec) {
    ...
    try {
      ...
      // FIX: Build outputs path dynamically using pathResolver or config.logsDir
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: this.buildCOSMOSystemPrompt(this.getAgentBehavioralPrompt()) + '\n\n' + 'You extract key concepts and important content from documents. Be selective and substantive.'
          },
          {
            role: 'user',
            content: prompt
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: this.getAgentBehavioralPrompt() + '\n\n' + 'You are an expert document analyst. Provide a concise summary of the document content in 2-3 sentences.'
          },
          {
            role: 'user',
...
Generate a narrative about how these documents evolved over time.`;
      ...
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: this.getAgentBehavioralPrompt() + '\n\n' + 'You are a document historian. Tell the story of how these documents evolved, their relationships, and key changes over time.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async storeDocumentContentsForHandoff(documents) {
    ...
    if (jsonSize > 50000) {
      ...
      for (const doc of documents) {
        ...
        const outputDir = path.join(runtimeRoot, 'agent-data');
        ...
        if (this.capabilities) {
          await this.capabilities.writeFile(
            dataPath,  // Use absolute path - pathResolver handles it correctly
  ...
  async createDeliverableOutput(report, deliverableSpec, documentCollection) {
    ...
    if (this.pathResolver) {
      outputDir = path.join(this.pathResolver.getOutputsRoot(), 'document-analysis', this.agentId);
    } else if (this.config?.logsDir) {
      outputDir = path.join(this.config.logsDir, 'outputs', 'document-analysis', this.agentId);
    } else {
