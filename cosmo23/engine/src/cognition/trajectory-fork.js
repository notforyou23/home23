...
class TrajectoryForkSystem {
  constructor(config, subsystems, logger) {
    ...
    // Forking criteria
    // Surprise threshold: 0.35 triggers on top ~25% of surprises (max possible is 0.5 with current calculation)
    // Uncertainty threshold: 0.6 is more achievable than previous 0.8
    this.surpriseThreshold = config.forking?.surpriseThreshold || 0.35;
    this.uncertaintyThreshold = config.forking?.uncertaintyThreshold || 0.6;
