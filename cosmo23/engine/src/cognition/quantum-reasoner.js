...
class QuantumReasoner {
  ...
  async generateSuperposition(prompt, context = {}) {
    ...
    this.lastPolicyDecision = {
      ...
      // Store branch metadata for later credit assignment
      branchIds: validHypotheses.map(h => h.branchId)
    };
  ...
  /**
   * Build branch-specific prompt with domain anchoring
   * 
   * CRITICAL FIX: Domain anchor MUST be injected here (not just in collapse phase)
   * to ensure all branch hypotheses are domain-relevant from generation.
   * This prevents generic "Research D / preregistration" contamination.
   */
  buildBranchPrompt(basePrompt, context, branchIndex, totalBranches) {
    ...
    // ✅ FIX: Get domain anchor (skip in pure mode to preserve minimal prompting)
    // This ensures all thoughts are domain-relevant, not generic research methodology
    const domainAnchorBlock = isPureMode ? '' : getDomainAnchor(this.fullConfig);
    ...
    // ✅ FIX: Build prompt WITH domain anchor FIRST (ensures domain context preserved)
    let prompt = '';
    if (!isPureMode && domainAnchorBlock) {
      // Domain anchor + perspective + base prompt
      prompt = `${domainAnchorBlock}\n\n${perspective}\n\n${basePrompt.substring(0, 150)}`;
    } else if (isPureMode) {
      // Pure mode: just base prompt (minimal)
      prompt = `${basePrompt.substring(0, 150)}`;
    } else {
      // Fallback: perspective + base (if domain anchor unavailable)
      prompt = `${perspective}\n\n${basePrompt.substring(0, 150)}`;
    }
  ...
  async collapseToBestGPT5(hypotheses, context) {
    // DOMAIN ANCHOR: Prefer domain-relevant hypotheses
    const domainAnchorBlock = getDomainAnchor(this.fullConfig);

    const evaluationPrompt = `${domainAnchorBlock}

Evaluate these ${hypotheses.length} hypotheses and select the SINGLE BEST one based on depth, coherence, and insight FOR THE DOMAIN RESEARCH.
...
⚠️ PREFER hypotheses related to the DOMAIN. Deprioritize hypotheses about QA gates, probes, CLI tools, COSMO infrastructure.
...
Respond with ONLY the number (1-${hypotheses.length}) of the best hypothesis.`;
  ...
  async singleReasoning(prompt, context = {}) {
    ...
    return {
      ...
      reasoning: response.reasoning,
      weight: 1.0,
      model: response.model || this.config?.models?.primary,
  ...
  async recordPolicyOutcome({ reward, selectedBranchId = null }) {
    ...
    // Find which branch was selected (for credit assignment)
    let selectedBranchIndex = null;
    if (selectedBranchId && Array.isArray(this.lastPolicyDecision.branchIds)) {
      selectedBranchIndex = this.lastPolicyDecision.branchIds.indexOf(selectedBranchId);
      if (selectedBranchIndex === -1) {
        selectedBranchIndex = null; // Branch not found in list
      }
    }
