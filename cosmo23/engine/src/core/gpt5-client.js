...
class GPT5Client {
  ...
  /**
   * Get the current OpenAI client, refreshing OAuth credentials if needed.
   * No-op when OPENAI_OAUTH_ENABLED is not set — returns this.client unchanged.
   * Updates this.client in place so the rest of generate() continues to use it.
   */
  async _getClient() {
  ...
  async generate(options = {}) {
    ...
    if (effectiveMaxTokens && !isMultiAgent) {
    ...
    const supportsReasoningEffort = model.includes('gpt-5') || model.includes('multi-agent') || model.includes('grok-4.20');
    if (reasoning) {
    ...
    try {
      ...
      // Check if we have function calls even without text content
      // This is valid for tool-use responses where the LLM only returns function calls
      const hasFunctionCalls = finalResponse?.output?.some(item => 
        item.type === 'function_call' || item.type === 'tool_use'
      );

      // If we STILL have no content after all fallbacks AND no function calls, that's a real problem
      if ((!aggregatedText || aggregatedText.length === 0) && !hasFunctionCalls) {
        ...
        this.logger?.error?.(errorMsg, {
          ...
          outputTypes: finalResponse?.output?.map(o => o.type)
        });
      ...
      if ((!aggregatedText || aggregatedText.length === 0) && hasFunctionCalls) {
        this.logger?.debug?.('Response has function calls but no text content (valid tool-use response)', {
          responseId: finalResponse?.id,
          functionCallCount: finalResponse?.output?.filter(o => o.type === 'function_call' || o.type === 'tool_use').length
        });
