...
class AnthropicClient {
  ...
  async generate(options = {}) {
    ...
    try {
      ...
      // Prefer options.messages (Chat Completions format) over options.input (Responses API)
      // when messages contain tool_calls. The Responses API input format uses flat
      // function_call items that _transformInputToMessages can't convert to tool_use blocks.
      // Chat Completions format (assistant.tool_calls + tool role) converts correctly.
      const hasToolMessages = Array.isArray(options.messages) &&
        options.messages.some(m => (m.role === 'assistant' && m.tool_calls) || m.role === 'tool');
      let messagesToUse = hasToolMessages ? options.messages : (options.input || options.messages);
