...
class Capabilities {
  ...
  /**
   * Unified action interface - COSMO's direct hands
   * 
   * Routes to appropriate method based on action type.
   * All existing executive gating, frontier logging, and safety remain intact.
   * 
   * @param {Object} action - Action specification
   * @param {string} action.type - Action type (read, write, edit_range, search_replace, run, grep, list, etc.)
   * @param {Object} agentContext - Context for executive gating (agentId, agentType, missionGoal, cycleCount)
   * @returns {Object} - Action result with { success, ...data, duration, actionType }
   */
  async do(action, agentContext = {}) {
    // Validate action
    ...
    try {
      ...
      switch (action.type) {
        case 'read':
          result = await this.readFile(action.path, agentContext);
          break;
          
        case 'write':
        case 'create':
          result = await this.writeFile(action.path, action.content, agentContext);
          break;
          
        case 'append':
          result = await this.appendFile(action.path, action.content, agentContext);
          break;
          
        case 'read_binary':
          result = await this.readFileBinary(action.path, agentContext);
          break;
          
        case 'edit_range':
          result = await this.editFileRange(
            action.path, 
            action.startLine, 
            action.endLine, 
            action.newContent, 
        ...
        case 'search_replace':
          result = await this.searchReplace(
            action.path, 
            action.oldString, 
            action.newString, 
            agentContext
        ...
        case 'run':
        case 'terminal':
          result = await this.runTerminal(action.command, agentContext, action.options);
          break;
          
        case 'grep':
          result = await this.grepSearch(action.pattern, action.path, agentContext);
          break;
          
        case 'semantic_search':
        case 'search':
          result = await this.semanticSearch(action.query, action.scope, agentContext);
          break;
          
        case 'list':
        case 'list_directory':
          result = await this.listDirectory(action.path, agentContext);
          break;
          
        case 'delete':
          result = await this.deleteFile(action.path, agentContext);
          break;
  ...
  /**
   * Delete file (executive-gated)
   * @param {string} logicalPath - Logical or absolute path
   * @param {Object} agentContext - Agent context
   * @returns {Object} - { success: boolean, reason?: string }
   */
  async deleteFile(logicalPath, agentContext = {}) {
    if (!this.enabled) {
      return { success: false, reason: 'Capabilities disabled' };
    }
    
    const actionId = `delete_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    this.pendingActions.add(actionId);
    this.stats.actionsAttempted++;
