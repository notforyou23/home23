    ...
    try {
      ...
      if (this.config.allowedPaths && Array.isArray(this.config.allowedPaths)) {
        ...
        if (name === 'read_file' || name === 'list_directory') {
          ...
          // Use runtimeRoot from config if available, otherwise fall back to __dirname
          // runtimeRoot should be set by the worker/orchestrator to the user's brain path
          const runtimeRoot = this.config.runtimeRoot || path.resolve(__dirname, '..', '..');
          
          // Resolve the requested path (supports both absolute and relative)
          // Strip 'runtime/' prefix if present, since runtimeRoot IS the runtime directory
          let pathToResolve = requestedPath;
          if (requestedPath.startsWith('runtime/')) {
            pathToResolve = requestedPath.replace('runtime/', '');
          }
          const resolvedRequested = path.isAbsolute(requestedPath)
            ? path.resolve(requestedPath)
            : path.resolve(runtimeRoot, pathToResolve);
          
          // Check if path is allowed
          const isAllowed = this.config.allowedPaths.some(allowedPath => {
            // Resolve allowed path using runtimeRoot
            // Handle 'runtime/...' paths by stripping the prefix
            let resolvedAllowed;
            if (allowedPath.startsWith('runtime/')) {
              resolvedAllowed = path.resolve(runtimeRoot, allowedPath.replace('runtime/', ''));
            } else if (path.isAbsolute(allowedPath)) {
              resolvedAllowed = path.resolve(allowedPath);
            } else {
              resolvedAllowed = path.resolve(runtimeRoot, allowedPath);
            }
