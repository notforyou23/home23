...
// OAuth PKCE Flow Constants (from anthropic-oauth-flow.md)
...
async function importFromClaudeCLI() {
  ...
  try {
    ...
    await storeToken(claudeAuth.access_token, expiresAt);
...
async function storeToken(token, expiresAt = null, refreshToken = null) {
  try {
    ...
    console.log('[OAuth-Engine] Token stored successfully');
    if (expiresAt) {
      console.log(`[OAuth-Engine] Expires: ${new Date(expiresAt).toISOString()}`);
    }
    if (refreshToken) {
      console.log('[OAuth-Engine] Refresh token stored');
    }

    return true;
  } catch (error) {
    console.error('[OAuth-Engine] Error storing token:', error.message);
    return false;
  }
}
...
async function clearToken() {
  try {
    ...
    console.log('[OAuth-Engine] Token cleared');
    return true;
  } catch (error) {
    if (error.code === 'P2025') {
    ...
    console.error('[OAuth-Engine] Error clearing token:', error.message);
    return false;
  }
}
  ...
  try {
    ...
    if (stored && stored.token) {
      ...
      if (isExpired && stored.refreshToken) {
        console.log('[OAuth-Engine] Access token expired, refreshing...');
        ...
        try {
          ...
          // Use refreshed token
          console.log('[OAuth-Engine] Token refreshed successfully');
          return {
