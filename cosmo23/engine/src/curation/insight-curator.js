  ...
  async scoreInsights(insights, options = {}) {
    ...
    for (let i = 0; i < batches.length; i += parallelBatches) {
      ...
      const promises = batchGroup.map(async ({ insights: batch, index }) => {
        // DOMAIN ANCHOR: Score insights for domain relevance
      const domainAnchorBlock = getDomainAnchor(this.config);
      
      const scoringPrompt = `You are evaluating research insights for knowledge synthesis and understanding.

${domainAnchorBlock}

Score each on 4 dimensions (0-10):
...
4. RESEARCH_CONTRIBUTION: Advances understanding and reveals knowledge connections? (10 = fundamental insight/connection, 0 = isolated fact)

⚠️ DOMAIN FILTER: Score insights about QA gates, probes, CLI tools, COSMO infrastructure as 0 on ALL dimensions (these are meta-pollution).
...
Return ONLY a JSON array: [{"index": 1, "actionability": X, "specificity": X, "novelty": X, "researchContribution": X}, ...]`;
        ...
        try {
          ...
          try {
            ...
            return batch.map(ins => ({
              ...ins,
              scores: { actionability: 5, specificity: 5, novelty: 5, researchContribution: 5 },
              totalScore: 20
            }));
          }
          ...
          return batch.map((insight, j) => {
            ...
            return {
              ...
              scores: {
                ...
                researchContribution: score.researchContribution || 5
              },
              totalScore: (score.actionability || 5) + (score.specificity || 5) +
                         (score.novelty || 5) + (score.researchContribution || 5)
            };
          });
  ...
  filterAndRank(scoredInsights) {
    // Filter for high-value insights (research-focused)
    const filtered = scoredInsights.filter(ins => {
      // Prioritize novelty, depth, and research contribution
      return ins.scores.researchContribution >= 5 ||
             ins.scores.novelty >= 5 ||
             ins.scores.specificity >= 6 ||
  ...
  async categorizeInsights(insights) {
    // DOMAIN ANCHOR: Categorize only domain-relevant insights
    const domainAnchorBlock = getDomainAnchor(this.config);
    
    const categorizationPrompt = `You are categorizing research insights for knowledge synthesis and research advancement.

${domainAnchorBlock}

Categories:
- TECHNICAL: Specific technical details, algorithms, constraints, implementation specifics FOR THE DOMAIN
- THEORETICAL: Conceptual frameworks, theoretical models, foundational principles, paradigm shifts FOR THE DOMAIN
- OPERATIONAL: Implementation approaches, architectures, workflows, processes FOR THE DOMAIN
- KNOWLEDGE_SYNTHESIS: Research gaps, theoretical gaps, areas requiring investigation, emerging connections FOR THE DOMAIN
- CROSS_DOMAIN: Connections between different fields, interdisciplinary insights RELATED TO THE DOMAIN
- META_POLLUTION: Insights about COSMO internals, QA gates, probes, CLI tools (SKIP THESE)

Insights to categorize:
${insights.map((ins, idx) => `\n${idx + 1}. ${ins.content.substring(0, 200)}`).join('\n')}

For each insight, provide:
1. Category (one of the 6 above - use META_POLLUTION for non-domain insights)
...
Return as JSON array: [{"index": 1, "category": "TECHNICAL", "title": "Brief specific title"}, ...]`;
    ...
    try {
      const response = await this.gpt5.generate({
        model: this.config.models?.curatorModel || this.config.models?.fast, // Use mini for faster categorization
      ...
      insights.forEach((ins, idx) => {
        ...
        else if (category.includes('theoretical')) categorized.theoretical.push(enriched);
        else if (category.includes('operational')) categorized.operational.push(enriched);
        else if (category.includes('knowledge')) categorized.knowledgeSynthesis.push(enriched);
  ...
  async generateExecutiveSummary(categorizedInsights, systemContext = {}) {
    const allInsights = [
      ...
      ...categorizedInsights.theoretical,
      ...categorizedInsights.operational,
      ...categorizedInsights.knowledgeSynthesis,
    ...
    // DOMAIN ANCHOR: Focus summaries on domain research
    const domainAnchorBlock = getDomainAnchor(this.config);
    
    if (mode === 'goal-aligned') {
      summaryPrompt = `You are analyzing research insights to create an executive summary focused on goal advancement.

${domainAnchorBlock}
${goalsContext}${directivesContext}
...
Theoretical Insights (${categorizedInsights.theoretical.length}):
${categorizedInsights.theoretical.slice(0, 5).map(i => `• ${i.content.substring(0, 150)}`).join('\n')}
...
Knowledge Synthesis (${categorizedInsights.knowledgeSynthesis.length}):
${categorizedInsights.knowledgeSynthesis.slice(0, 5).map(i => `• ${i.content.substring(0, 150)}`).join('\n')}

Write a concise 2-paragraph executive summary (under 300 words) that:
1. Highlights how these insights advance the DOMAIN RESEARCH goals (ignore meta-goals about COSMO)
2. Identifies alignment with strategic directives FOR THE DOMAIN
3. Recommends next steps for DOMAIN RESEARCH progression
4. Notes DOMAIN knowledge gaps that need addressing

⚠️ IGNORE insights about QA gates, probes, CLI tools, COSMO infrastructure.
Focus on goal advancement and strategic alignment FOR THE USER'S RESEARCH DOMAIN.`;
...
Write a concise 2-paragraph executive summary (under 300 words) that:
1. Highlights key DOMAIN findings and their significance
2. Identifies novel discoveries and research directions IN THE DOMAIN
3. Recommends next DOMAIN research steps
4. Notes important open questions FOR THE DOMAIN

⚠️ IGNORE insights about QA gates, probes, CLI tools, COSMO infrastructure.
Focus on research value and scientific advancement IN THE USER'S RESEARCH DOMAIN.`;
    } else {
...
Write a concise 2-paragraph executive summary (under 300 words) that:
1. Highlights the key DOMAIN findings and their business implications
2. Identifies the most actionable DOMAIN recommendations
3. Outlines strategic priorities FOR THE DOMAIN RESEARCH

⚠️ IGNORE insights about QA gates, probes, CLI tools, COSMO infrastructure.
Focus on business value and actionable insights FOR THE USER'S RESEARCH DOMAIN.`;
    }

    try {
      const response = await this.gpt5.generate({
        model: this.config.models?.curatorStrategic || this.config.models?.strategicModel,
        input: summaryPrompt,
        maxTokens: 16000, // API maximum output limit (128K input + 16K output)
        reasoningEffort: 'high', // Executive synthesis for research understanding deserves deep reasoning
      ...
      return `This research identified ${allInsights.length} high-value insights across technical, theoretical, and operational domains. Key findings include: ${topInsights.map(i => i.title || i.content.substring(0, 100)).join('; ')}. These insights present significant opportunities for knowledge advancement and research understanding.`;
    }
  }
  ...
  async analyzeGoalAlignment(categorizedInsights, systemContext = {}) {
    ...
    // DOMAIN ANCHOR: Focus goal alignment on domain research
    const domainAnchorBlock = getDomainAnchor(this.config);

    const analysisPrompt = `You are analyzing research insights for goal alignment and strategic value.
...
For the top 5-8 most valuable DOMAIN-RELEVANT insights, analyze their strategic value:
1. Which insight (by number)
2. Which DOMAIN goal(s) does this insight advance? (use goal IDs if available - skip meta-goals about COSMO)
3. How does it advance that DOMAIN goal? (specific contribution)
4. What's the next step to capitalize on this insight FOR THE DOMAIN RESEARCH?
5. Priority level (high/medium/low) based on DOMAIN goal alignment

⚠️ SKIP insights about QA gates, probes, CLI tools, COSMO infrastructure - these are meta-pollution.

Return your analysis as a JSON array with this exact structure:
[{"insight": 1, "relatedGoals": ["goal_123"], "contribution": "How it advances the goal", "nextStep": "Specific action", "priority": "high"}, ...]

Focus on DOMAIN goal advancement and actionable next steps FOR THE USER'S RESEARCH.`;
  ...
  async analyzeResearchDirections(categorizedInsights, systemContext = {}) {
    ...
    // DOMAIN ANCHOR: Focus research directions on domain
    const domainAnchorBlock = getDomainAnchor(this.config);
...
For the top 5-8 most scientifically valuable DOMAIN-RELEVANT insights, identify research directions:
1. Which insight (by number)
2. Research question it raises FOR THE DOMAIN
3. Methodology to explore further IN THE DOMAIN
4. Expected scientific impact FOR THE DOMAIN
...
[{"insight": 1, "researchQuestion": "...", "methodology": "...", "impact": "...", "priority": "high"}, ...]`;
  ...
  async analyzeBusinessOpportunities(categorizedInsights) {
    ...
    // DOMAIN ANCHOR: Focus commercial analysis on domain
    const domainAnchorBlock = getDomainAnchor(this.config);
...
For the top 5-8 most commercially valuable DOMAIN-RELEVANT insights, identify potential consulting services or projects:
1. Specific consulting service or project name IN THE DOMAIN
2. Estimated project value ($50K, $500K, $1M, $5M+)
3. Target client type who would pay for this DOMAIN expertise
...
**Actionability:** ${ins.scores.actionability}/10 | **Research Value:** ${ins.scores.researchContribution}/10 | **Novelty:** ${ins.scores.novelty}/10
...
## Theoretical Insights (${curationResults.topInsights.theoretical.length})

${curationResults.topInsights.theoretical.map((ins, idx) => `
### ${idx + 1}. ${ins.title || 'Theoretical Insight'}

**Actionability:** ${ins.scores.actionability}/10 | **Research Value:** ${ins.scores.researchContribution}/10
...
## Knowledge Synthesis (${curationResults.topInsights.knowledgeSynthesis.length})

${curationResults.topInsights.knowledgeSynthesis.map((ins, idx) => `
### ${idx + 1}. ${ins.title || 'Knowledge Synthesis'}
