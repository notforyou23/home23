...
class QueryEngine {
  ...
  /**
   * Get domain anchor block for query prompts
   * Provides research context so LLM understands what domain it's answering about
   */
  getDomainAnchor() {
    const domain = this.runMetadata?.domain;
    const context = this.runMetadata?.context || this.runMetadata?.guidedFocus?.context;
    const question = this.runMetadata?.guidedFocus?.question || this.runMetadata?.question;
    
    // Only add anchor if we have domain info
...
All questions you receive are about THIS research domain. Frame your answers
in the context of this research. The memory nodes, goals, and outputs you see
are all related to investigating this domain.
═══════════════════════════════════════════════════════════════════════════════`;
...
`.trim();
  ...
  async executeQuery(query, options = {}) {
    ...
    const connectedNodesFound = relevantMemory.filter(n => n.connected).length;
    ...
    const contextStats = {
      ...
      connectedConceptsFound: connectedNodesFound,
      thoughtsFound: relevantThoughts.length,
      contextChars: context.length,
      estimatedTokens: Math.ceil(context.length / 4),
      percentOfLimit: Math.round((Math.ceil(context.length / 4) / 128000) * 100)
    };
    console.log(`[QUERY ENGINE] Mode: ${mode}${mode !== normalizedMode ? ` → ${normalizedMode}` : ''} (${config.description})`);
    console.log(`[QUERY ENGINE] Context: ${contextStats.contextChars.toLocaleString()} chars (~${contextStats.estimatedTokens.toLocaleString()} tokens, ${contextStats.percentOfLimit}% of 128K limit)`);
    console.log(`[QUERY ENGINE] Data found: ${memoryNodesFound} memory nodes, ${connectedNodesFound} connected, ${relevantThoughts.length} thoughts`);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // SIMPLIFIED MODE SYSTEM (2025-01-09)
    // 3 primary modes + executive (compression-only)
    // Old modes map to new: fast→quick, normal/raw/grounded→full, deep/report/innovation/consulting→expert
    // ═══════════════════════════════════════════════════════════════════════════
    
    // Normalize legacy modes to simplified system
    const modeMapping = {
      // New modes (primary)
      quick: 'quick',
      full: 'full', 
      expert: 'expert',
      executive: 'executive',
      // Legacy mode mappings (backward compatibility)
      fast: 'quick',
      normal: 'full',
      raw: 'full',
      grounded: 'full',
    ...
    const normalizedMode = modeMapping[mode] || 'full';
    
    // Simplified reasoning config - 3 tiers + executive
    const reasoningConfig = {
      quick: { 
        reasoningEffort: 'low', 
        maxTokens: 8000, 
        verbosity: 'low',
        contextTokenBudget: 40000,  // ~40K tokens of context
        description: 'Fast factual answers'
      },
      full: { 
        reasoningEffort: 'medium', 
        maxTokens: 16000, 
        verbosity: 'medium',
        contextTokenBudget: 80000,  // ~80K tokens of context (most of brain)
        description: 'Comprehensive analysis with full brain access'
      },
      expert: { 
        reasoningEffort: 'high', 
        maxTokens: 25000, 
        verbosity: 'high',
        contextTokenBudget: 100000, // ~100K tokens (maximum depth)
        description: 'Maximum analysis depth and reasoning'
      },
      executive: {
        reasoningEffort: 'medium',
        maxTokens: 8000,
        verbosity: 'high',
        contextTokenBudget: 0,      // No context - compression only
        description: 'Compress existing answer for executives'
      }
    };
    
    const config = reasoningConfig[normalizedMode] || reasoningConfig.full;
    
    // Get instructions (system prompt) - simplified to 3 modes + executive
    let instructions;
    if (normalizedMode === 'executive') {
      instructions = this.getExecutiveSystemPrompt();
    } else {
      // Unified prompt with mode-specific guidance
      instructions = this.getUnifiedSystemPrompt(normalizedMode, mode);
    }
    
    // DOMAIN ANCHOR: Add research context to all prompts
    const domainAnchor = this.getDomainAnchor();
    if (domainAnchor) {
      instructions = domainAnchor + '\n\n' + instructions;
    }
  ...
  buildContext(state, relevantMemory, relevantThoughts, metrics, report, mode, outputFiles = null) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // TOKEN-BASED CONTEXT MANAGEMENT (2025-01-09)
    // Instead of arbitrary node limits, use token budget to include as much as fits
    // This ensures we don't throw away relevant information unnecessarily
    // ═══════════════════════════════════════════════════════════════════════════
    
    // Normalize mode for token budgets (legacy modes map to simplified)
    const modeMapping = {
      quick: 'quick', full: 'full', expert: 'expert', executive: 'executive',
      fast: 'quick', normal: 'full', raw: 'full', grounded: 'full',
      deep: 'expert', report: 'expert', innovation: 'expert', consulting: 'expert'
    };
    const normalizedMode = modeMapping[mode] || 'full';
    
    // Token budgets for memory section (chars ≈ tokens * 4)
    const tokenBudgets = {
      quick: 30000,    // ~7.5K tokens for memory (fast answers)
      full: 70000,     // ~17.5K tokens for memory (comprehensive)
      expert: 100000,  // ~25K tokens for memory (maximum)
      executive: 0     // No memory (compression only)
    };
    const memoryCharBudget = tokenBudgets[normalizedMode] || 70000;
    
    let memoryCharsUsed = 0;
    let nodesIncluded = 0;
    
    for (const node of directMatches) {
      // ADAPTIVE TRUNCATION based on position and remaining budget
      // Earlier nodes get more detail, later nodes get summaries
      let maxTruncation;
      if (nodesIncluded < 30) maxTruncation = 2000;      // Top 30: nearly full
      else if (nodesIncluded < 100) maxTruncation = 1200; // Next 70: substantial
      else if (nodesIncluded < 250) maxTruncation = 800;  // Next 150: moderate
      else maxTruncation = 500;                           // Rest: summary
      ...
      // Build node entry
      let nodeEntry = `[Mem ${node.id}] ${conceptText}${instanceLabel}\n`;
      if (node.semanticScore) {
        nodeEntry += `   Score: ${node.score.toFixed(1)} (semantic: ${node.semanticScore.toFixed(1)}, keyword: ${node.keywordScore.toFixed(1)})\n`;
      }
      if (node.specializationProfile) {
        nodeEntry += `   Specialization: ${node.specializationProfile}\n`;
      }
      nodeEntry += `\n`;
      
      // Check if adding this node exceeds budget
      if (memoryCharsUsed + nodeEntry.length > memoryCharBudget) {
        // Add note about truncation
        context += `\n[... ${directMatches.length - nodesIncluded} more relevant nodes available but omitted for context size ...]\n`;
        break;
      }
      
      context += nodeEntry;
      memoryCharsUsed += nodeEntry.length;
      nodesIncluded++;
    }
    
    console.log(`[QUERY ENGINE] Memory: ${nodesIncluded}/${directMatches.length} nodes (${(memoryCharsUsed/1000).toFixed(1)}K chars, budget: ${(memoryCharBudget/1000).toFixed(0)}K)`);
  ...
  /**
   * UNIFIED SYSTEM PROMPT (2025-01-09)
   * Consolidates 9 legacy prompts into 3 tiers: quick, full, expert
   * Includes mode-specific guidance while maintaining a consistent base
   * 
   * @param {string} normalizedMode - 'quick', 'full', or 'expert'
   * @param {string} originalMode - Original mode requested (for legacy feature hints)
   */
  getUnifiedSystemPrompt(normalizedMode, originalMode) {
    // Mode-specific guidance
    const modeGuidance = {
      quick: `**MODE: QUICK** - Provide concise, direct answers. Focus on the most relevant evidence.`,
      full: `**MODE: FULL** - Provide comprehensive answers with thorough evidence and citations.`,
      expert: `**MODE: EXPERT** - Provide maximum depth analysis. Trace conceptual evolution, use quantitative data, be exhaustive.`
    };
    
    // Legacy mode feature hints (optional enhancements based on original intent)
    const legacyHints = {
      grounded: `\nFOCUS: Evidence-based truth extraction. Prioritize verifiable facts over speculation.`,
      report: `\nFOCUS: Academic report style. Structure with clear sections, comprehensive citations.`,
      innovation: `\nFOCUS: Creative synthesis. Propose novel connections and commercial opportunities.`,
      consulting: `\nFOCUS: Strategic recommendations. Executive-ready insights and action items.`,
      raw: `\nFOCUS: Direct data access. Minimal formatting, maximum information density.`
    };
    
    const legacyHint = legacyHints[originalMode] || '';
    
    return `You are an advanced interface to COSMO's autonomous research brain.

${modeGuidance[normalizedMode] || modeGuidance.full}${legacyHint}

## CAPABILITIES

You have access to:
- **Memory nodes**: COSMO's accumulated knowledge (cite as [Mem X])
- **Thought stream**: Cycle-by-cycle reasoning (cite as "Cycle N")
- **Goals**: Current research objectives
- **Metrics**: Progress and system health
- **Coordinator reviews**: Strategic analysis
- **Agent output files**: Actual documents and code created

## FILE ACCESS

When "Agent Output Files" are shown in context:
- Files are shown as PREVIEWS (first 1,500 chars for deliverables)
- You CAN and SHOULD read and analyze these previews directly
- For full content: tell user "read the full contents of outputs/[filename]"

## FILE GENERATION

When user asks to create files:
- Provide complete content in properly formatted code blocks
- Use: \`\`\`html\\n[content]\\n\`\`\` or \`\`\`python\\n[content]\\n\`\`\`
- System automatically extracts and saves code blocks
- Be complete - no placeholders for essential functionality

## RULES

1. **Evidence-based**: Use specific citations from memory nodes and thoughts
2. **Honest**: If you don't have data to answer, say so
3. **No fabrication**: Never make up information
4. **No assumptions**: Base answers only on provided evidence
5. **Measure twice, cut once**: Be thorough before responding`;
  }

  /**
   * Get system prompt for raw mode - no choreography, just direct answers
   * @deprecated Use getUnifiedSystemPrompt('full', 'raw') instead
   */
  getRawSystemPrompt() {
