...
class IntelligenceBuilder {
   ...
   */
  getRunDir(runName) {
    return runName === 'runtime' ? this.defaultRunDir : path.join(this.runsDir, runName);
  }
  ...
  async loadLatestInsight(runName) {
    ...
    try {
      ...
      const insights = files
        .filter(f => f.startsWith('insights_curated_') && f.endsWith('.md') && f.includes('cycle_'))
        .sort((a, b) => {
          const numA = parseInt(a.match(/cycle_(\d+)/)?.[1] || 0);
          const numB = parseInt(b.match(/cycle_(\d+)/)?.[1] || 0);
          return numB - numA;
        });
  ...
  /**
   * Load ALL curated insights across all cycles
   */
  async loadAllInsights(runName) {
    const runDir = this.getRunDir(runName);
    const coordinatorDir = path.join(runDir, 'coordinator');
    ...
    try {
      ...
      for (const file of insightFiles) {
        ...
        if (parsed && parsed.insights) {
          const cycleMatch = file.match(/cycle_(\d+)/);
          const cycleNum = cycleMatch ? parseInt(cycleMatch[1]) : 0;
          parsed.insights.forEach(insight => {
            insight.cycle = cycleNum;
            insight.sourceFile = file;
          });
  ...
  parseInsightFile(content) {
    ...
    for (const section of sections) {
      ...
      // Also supports "Research Value" label (renamed from "Strategic Value" in curator refactor)
      ...
      const strategicMatch = fullText.match(/\*\*(?:Strategic Value|Research Value):\*\*\s*(\d+)\/10/);
      ...
      insight.summary = content.substring(0, 200);
  ...
  async buildIntelligenceSummary(runName) {
    try {
      const [state, latestInsight, latestReview, allInsights] = await Promise.all([
        this.loadState(runName),
        this.loadLatestInsight(runName),
        this.loadLatestReview(runName),
        this.loadAllInsights(runName)
      ]);

      if (!state) {
        return {
          topDiscovery: 'Research state not available yet',
          confidence: 'UNKNOWN',
          validated: 0,
          investigating: 0,
          nextDirection: 'Waiting for run state',
      ...
      const validated = activeGoals.filter(item => {
        const goal = Array.isArray(item) ? item[1] : item;
        return goal.status === 'completed';
      }).length;
      const investigating = activeGoals.filter(item => {
        const goal = Array.isArray(item) ? item[1] : item;
  ...
  async extractTopDiscoveries(runName, count = 5) {
    try {
      const allInsights = await this.loadAllInsights(runName);
      if (allInsights.length === 0) {
        return [];
      }

      // Sort by composite score (avg of all three metrics)
      const scored = allInsights.map(insight => {
        const composite = (
          (insight.scores.actionability || 0) +
          (insight.scores.strategic || 0) +
          (insight.scores.novelty || 0)
        ) / 3;
        return { ...insight, compositeScore: composite };
      });
