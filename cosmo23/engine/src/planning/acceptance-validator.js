...
class AcceptanceValidator {
  ...
  async checkQA(criterion, artifacts) {
    ...
    try {
      ...
      // DOMAIN ANCHOR: Evaluate against domain criteria
      const domainAnchorBlock = getDomainAnchor(this.agentExecutor?.config || {});
...
Evaluate whether the task meets the rubric FOR THE DOMAIN RESEARCH. Respond with:
- PASS if the task meets the DOMAIN criteria (score >= ${threshold})
- FAIL if the task does not meet the DOMAIN criteria
- FAIL if the task is META-POLLUTION (about QA gates, probes, CLI tools, COSMO infrastructure instead of domain research)
