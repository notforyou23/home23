...
class TopicQueueSystem {
  ...
  async enrichTopic(topicData) {
    try {
      // DOMAIN ANCHOR: Ensure topics relate to domain
      const domainAnchorBlock = getDomainAnchor(this.config);

      const response = await this.gpt5.generate({
        component: 'topicQueue',
        purpose: 'topicEnrichment',
        instructions: 'You are an expert at breaking down exploration topics into actionable investigation prompts and sub-goals FOR THE USER\'S RESEARCH DOMAIN.',
        messages: [{
          role: 'user',
          content: `${domainAnchorBlock}

Analyze this exploration topic:

Topic: "${topicData.topic}"
Context: ${topicData.context || 'none'}
Depth: ${topicData.depth || 'normal'}

Provide (MUST be DOMAIN-RELEVANT):
1. An enriched exploration prompt (2-3 sentences) that captures the essence FOR THE DOMAIN
2. 2-4 sub-goals or questions to investigate IN THE DOMAIN

⚠️ If the topic is about QA gates, probes, CLI tools, COSMO infrastructure, respond with: {"skip": true, "reason": "Meta-pollution"}
