...
class GoalCaptureSystem {
  ...
  detectSurprise(output) {
    ...
    for (const pattern of surpriseIndicators) {
      ...
      if (matches) surpriseScore += matches.length * 0.2;
    }

    // Semantic quality indicators (GPT-5 style)
    // Hypothesis elaboration - longer, more detailed thoughts
    const length = output.length;
    if (length > 200) surpriseScore += 0.15;
    if (length > 400) surpriseScore += 0.15;
    if (length > 600) surpriseScore += 0.1;

    // Questions indicate exploration
    const questionMarks = (output.match(/\?/g) || []).length;
    surpriseScore += Math.min(0.2, questionMarks * 0.1);

    // Conditional thinking (if/when/whether/could)
    const conditionals = (output.match(/\b(if|when|whether|could|might|may|perhaps|possibly)\b/gi) || []).length;
    surpriseScore += Math.min(0.15, conditionals * 0.03);
    ...
    if (vocabularyRatio > 0.6) surpriseScore += 0.15; // High diversity

    // Complex sentences (semicolons, em-dashes, parentheticals)
    const complexMarkers = (output.match(/[;—\(\)]/g) || []).length;
    surpriseScore += Math.min(0.1, complexMarkers * 0.05);
