...
class IntrinsicGoalSystem {
  constructor(config, logger, eventEmitter = null) {
    ...
    // Evaluation framework (set by orchestrator)
    this.evaluation = null;
  }

  /**
   * Set evaluation framework for metrics tracking
   * Called by orchestrator after initialization
   */
  setEvaluationFramework(evaluation) {
    this.evaluation = evaluation;
    this.logger?.info('Evaluation framework connected to IntrinsicGoalSystem');
  }

  /**
   * Get the event emitter for this goals context.
   */
  _getEvents() {
  ...
  async discoverViaReflection(journal) {
    ...
    // DOMAIN ANCHOR: Stronger domain focus for goal discovery
    const domainAnchorBlock = isGuided 
      ? getDomainAnchor({ domain: guidedDomain, context: guidedContext })
      : '';

    const domainGuidance = isGuided
      ? `\n\n⚠️ DOMAIN FILTER: ONLY generate goals related to "${guidedDomain}". ${guidedContext ? guidedContext + ' ' : ''}
Goals about QA gates, probes, CLI tools, COSMO infrastructure are META-POLLUTION - DO NOT include them.`
      : '';

    const prompt = `
${domainAnchorBlock}

Analyze these recent thoughts and identify 2-3 intrinsic goals or questions that emerge as worth pursuing FOR THE DOMAIN RESEARCH:

${recentThoughts}${domainGuidance}

For each goal, provide:
1. A clear description (one sentence) - MUST relate to the research domain
2. Why it's interesting or important FOR THE DOMAIN
3. Estimated uncertainty (0-1, where 1 is highly uncertain)

⚠️ EXCLUDE goals about QA gates, probes, CLI tools, COSMO infrastructure - these are meta-pollution.
...
Format as JSON array: [{"description": "...", "reason": "...", "uncertainty": 0.8}]
  ...
  addGoal(goalData) {
    ...
    this._getEvents().emitGoalCreated({
      goalId: goal.id,
  ...
  adjustPriority(goal, context) {
    ...
    if (isTaskGoal) {
      ...
      if (executionMode === 'guided-exclusive' || executionMode === 'strict') {
      ...
      if (isGuidedMode) {
        // GUIDED MODE: Strategic goals are LOWER than task goals but still pursuable
        // FIX (Jan 21, 2026): Changed from 0.5 to 0.8 - 0.5 was pushing goals below
        // the archive threshold (0.3), causing premature archival
        priority *= 0.8;

        this.logger?.debug('Strategic goal adjusted in guided mode', {
          goalId: goal.id,
          source: goal.source,
          basePriority: goal.priority,
          adjustedPriority: priority,
          reason: 'Plan tasks have higher priority but goals still pursued'
        });
      } else {
  ...
  completeGoal(goalId, notes = '') {
    ...
    // Track goal completion in evaluation framework
    if (this.evaluation) {
      this.evaluation.trackGoalCompleted(goalId, {
        notes,
    ...
    this._getEvents().emitGoalCompleted({
      goalId: goalId,
      description: goal.description,
      outcome: 'completed'
    });
  ...
  archiveGoal(goalId, reason = 'archived') {
    ...
    // If archived due to synthesis, track as synthesized; otherwise track as abandoned
    if (this.evaluation) {
      if (reason && reason.includes('Synthesized into')) {
        this.evaluation.trackGoalSynthesized(goalId, {
          reason,
          synthesizedInto: reason.match(/Synthesized into (\w+)/)?.[1] || 'unknown'
        });
      } else {
        // Track as abandoned
        this.evaluation.metrics.goals.abandoned++;
      }
    }
