...
// CRITICAL: Get user-specific runtime path from environment (multi-tenant support)
const LOGS_DIR = process.env.COSMO_RUNTIME_PATH || null;

// Load COSMO configuration for file access control
// PRIORITY: User's runtime config.yaml > engine/src/config.yaml
let COSMO_CONFIG = null;
let ALLOWED_PATHS = null;
try {
  // First try user-specific runtime config
  const runtimeConfigPath = LOGS_DIR ? path.join(LOGS_DIR, 'config.yaml') : null;
  const staticConfigPath = path.join(ROOT_PATH, 'src', 'config.yaml');
  const configPath = (runtimeConfigPath && fsSync.existsSync(runtimeConfigPath))
    ? runtimeConfigPath
    : staticConfigPath;
  ...
  // CRITICAL: If LOGS_DIR is set, always allow access to it (multi-tenant)
  if (LOGS_DIR) {
    ALLOWED_PATHS = ALLOWED_PATHS || [];
    if (!ALLOWED_PATHS.includes(LOGS_DIR)) {
      ALLOWED_PATHS.push(LOGS_DIR);
      console.log('📁 Added user runtime path to allowed:', LOGS_DIR);
    }
  }
} catch (error) {
...
function isPathAllowed(relPath) {
  // Admin bypass — no path restrictions (still block node_modules/.git)
  if (process.env.COSMO_ADMIN_MODE === 'true') {
    return !isPathExcluded(relPath);
  }
