...
describe('Cosmo research Launch contract', () => {
  it('Launch destination is the Watch desk, never Interactive', () => {
...
describe('Short Launch plan', () => {
  ...
  it('preserves explicit launch phases as the first drill goal', async () => {
    const planner = createPlanner({
      config: {
        logsDir: '/tmp/cosmo-research-launch-test',
        architecture: {
          roleSystem: {
            explorationMode: 'guided',
            guidedFocus: {
              domain: 'Garcia partnership',
              context: [
                'PHASE 1 - Archive evidence',
                'Fetch and verify the source record.',
                'PHASE 2 - Partnership analysis',
                'Analyze the relationship evidence.',
                'PHASE 3 - Write the finding',
                'Land the conclusion in outputs/finding.md.'
              ].join('\n'),
              executionMode: 'guided-exclusive'
            }
          }
    ...
    const result = await planner.planMission({ forceNew: true });
    expect(result.shortPlan.seedPhases.map(phase => phase.title)).to.deep.equal([
      'Archive evidence',
      'Partnership analysis',
      'Write the finding'
    ]);
    expect(result.shortPlan.seedPhases[2].expectedOutput).to.equal('outputs/finding.md');
  });
});
