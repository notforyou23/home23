...
describe('Continuation-Aware Planning — PGS Assessment Integration', () => {
  ...
  it('fresh Launch does not block the drill on a PGS assessment', async () => {
  ...
  it('planMission is independent of a broken assessment subsystem', async () => {
...
describe('Continuation-Aware Planning — End-to-End Prompt Flow', () => {
  ...
  it('fresh Launch composes the short drill contract without an LLM planning call', async () => {
    ...
    expect(planningPromptSeen).to.be.null;
    expect(result.executionKind).to.equal('tool_loop');
    expect(result.shortPlan.goal).to.equal('Test Domain');
  });

  it('planMission without assessment still produces a runnable drill contract', async () => {
    let planningPromptSeen = null;
    ...
    expect(planningPromptSeen).to.be.null;
    expect(result.shortPlan).to.exist;
    expect(result.shortPlan.deliverable).to.be.a('string').and.not.empty;
  });

  it('contextRedirect regenerates the short drill contract without legacy continuation prompts', async () => {
    let planningPromptSeen = null;
