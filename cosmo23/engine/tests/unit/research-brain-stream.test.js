...
const { writeBrainStream, replayJournaledBrainStream } = require('../../src/agent/brain-stream');
...
function streamAt(runtimePath) {
  const rows = readJsonl(path.join(runtimePath, 'outputs', 'stream.jsonl'));
  const live = new Set(readJsonl(path.join(runtimePath, 'outputs', 'stream-brain.jsonl'))
    .filter(entry => entry.brain === 'live')
    .map(entry => entry.id));
  return rows.map(row => row.id && live.has(row.id) ? { ...row, brain: 'live' } : row);
}
...
describe('The desk sees the tape', () => {
  it('the drill status endpoint serves the stream and never lists the tape as a writeup', () => {
    ...
    expect(serverSource).to.include("readJsonlTape(runPath, 'stream'");
    expect(serverSource).to.include('stream: brainTape.entries');
    expect(serverSource).to.include("file.kind === 'writeup'");
  });

  it('the Brain panel renders the working stream, not only remember() candidates', () => {
    const appSource = fs.readFileSync(path.join(__dirname, '../../../public/app.js'), 'utf8');
    expect(appSource).to.include('status.stream');
    expect(appSource).to.include("replaceLedger('drill-brain-ledger'");
    expect(appSource).to.include('No tape yet');
    expect(appSource).to.include('No receipts yet');
  });
});
...
describe('The tape itself is degraded-honest', () => {
  ...
  it('replays journaled tape into the Brain and acknowledges it durably', async () => {
    const runtimePath = tempRuntime();
    await writeBrainStream(
      { runtimePath, memory: null, logger },
      { kind: 'thought', content: 'A crash-safe thought', goalNumber: 1, phaseNumber: 2 }
    );
    const memory = makeMemory();

    const replay = await replayJournaledBrainStream({ runtimePath, memory, logger });
    const replayAgain = await replayJournaledBrainStream({ runtimePath, memory, logger });

    expect(replay.replayed).to.equal(1);
    expect(replayAgain.replayed).to.equal(0);
    expect(memory.added).to.have.length(1);
    expect(memory.added[0].metadata.source).to.equal('drill_stream_replay');
    expect(streamAt(runtimePath)[0].brain).to.equal('live');
  });
