...
`;
    ...
    expect(plan.shortPlan.seedPhases[0].expectedOutput).to.equal('@outputs/raw-anecdotes/archive-org-comments.json');
...
`;
    ...
    expect(plan.shortPlan.seedPhases.map(phase => phase.expectedOutput)).to.deep.equal([
      '@outputs/raw-anecdotes/archive-org-comments.json',
    ...
    expect(storedTasks[0].metadata.seedPhases).to.have.length(3);
  });
