...
const { getAnthropicApiKey, prepareSystemPrompt, isOAuthToken, getStealthHeaders } = require('../server/services/anthropic-oauth.js');
const { normalizeProviderCompletion } = require('./provider-completion.js');
const {
  abortableDelay,
  ...
  _transformTools(tools) {
    ...
    return tools
      ...
      .map(tool => {
        ...
        const schema = func.parameters || func.input_schema || { type: 'object', properties: {} };
        // Ensure schema has required type field (Anthropic API requires type: 'object')
