...
const { getModelCapabilities } = require('../server/config/model-catalog.js');
...
function createResponsesClient({ provider, modelConfig, providerConfig, logger }) {
  ...
  const { GPT5Client } = require('./gpt5-client.js');
...
function createCodexResponsesClient({
  ...
  const { CodexResponsesClient } = require('./codex-responses-client.js');
