...
const { encryptApiKey, decryptApiKey } = require('./encryption.js');
...
/**
 * Import token from Claude CLI credentials
 * Checks both ~/.claude/.credentials.json (current) and ~/.claude/auth.json (legacy)
 */
async function importFromClaudeCLI() {
  const credentialsPath = path.join(os.homedir(), '.claude', '.credentials.json');
  const legacyAuthPath = path.join(os.homedir(), '.claude', 'auth.json');
  ...
  if (fs.existsSync(credentialsPath)) {
    try {
      ...
      if (!oauth?.accessToken) {
        return { success: false, error: 'No OAuth token found in Claude CLI credentials' };
      }
      accessToken = oauth.accessToken;
      refreshToken = oauth.refreshToken || null;
      expiresAt = oauth.expiresAt || null;
    } catch (error) {
      return { success: false, error: `Failed to parse Claude CLI credentials: ${error.message}` };
    }
  } else if (fs.existsSync(legacyAuthPath)) {
    try {
      const claudeAuth = JSON.parse(fs.readFileSync(legacyAuthPath, 'utf-8'));
      if (!claudeAuth.access_token) {
        return { success: false, error: 'No access token found in Claude CLI auth' };
      }
      accessToken = claudeAuth.access_token;
      refreshToken = claudeAuth.refresh_token || null;
      expiresAt = claudeAuth.expires_at ? new Date(claudeAuth.expires_at).getTime() : null;
    } catch (error) {
      return { success: false, error: `Failed to parse Claude CLI auth: ${error.message}` };
    }
  } else {
    return { success: false, error: 'Claude CLI credentials not found. Make sure you are logged in to Claude Code.' };
  }

  await storeToken(accessToken, expiresAt, refreshToken);

  return {
    success: true,
    expiresAt: expiresAt ? new Date(expiresAt).toISOString() : null,
    isOAuth: isOAuthToken(accessToken)
  };
}
    ...
    if (stored && stored.token) {
      ...
      if (isExpired && stored.refreshToken) {
        ...
        try {
          ...
          // Fall through to error — OAuth-only, no API key fallback
        }
