...
export async function assembleMacRelease({ payload, appleSource, appleCommit, output, developerDir }) {
  ...
  phase('Building Host with the verified runtime');
  const hostBuild = path.join(build, 'host');
  run(process.execPath, [path.join(appleSource, 'scripts/build-home23-host.mjs'), '--output', hostBuild, '--payload', payload], { env, stdio: 'inherit' });
  const hostReceipt = json(path.join(hostBuild, 'build-receipt.json'));
  phase('Building the Mac client from the same Apple revision');
  ...
  const hostApp = path.join(release, 'Home23 Host.app'), clientApp = path.join(release, 'Home23.app');
  // Both builds are in this new output directory. Renames avoid another full runtime copy.
  ...
  const host = appInfo(hostApp), client = appInfo(clientApp);
  ...
  fs.writeFileSync(path.join(release, 'Start Here.txt'), `Home23 — local engineering candidate\n\nRequires macOS ${minimumMacOS} or later on ${manifest.arch}.\nHost alone requires macOS ${host.minimumMacOS}; the included conversation app requires ${client.minimumMacOS}.\n\nKeep both apps together, or copy both to Applications when you choose to install.\nOpen Home23 Host to create or select your home. Connect your own provider, then\nuse Open Home23 or Open Dashboard. The apps use the same home; do not create\na second home to connect the conversation app.\n\nThis candidate is ad-hoc signed, not notarized or publicly released.\nThis assembly does not install apps or change an existing home.\nDo not bypass macOS security warnings to treat it as a consumer release.\n`);
  ...
  const descriptor = { schema: 'home23.mac-release-set.v1', status: 'local-engineering-candidate',
    ...
    signing: 'ad-hoc', clientSandbox: true, clientProfileCapabilities: 'APNs and time-sensitive notifications excluded from local ad-hoc build', notarized: false, published: false, installed: false,
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    const names = { '--payload': 'payload', '--apple-source': 'appleSource', '--apple-commit': 'appleCommit', '--output': 'output', '--developer-dir': 'developerDir' };
    const options = {};
    ...
    for (const key of Object.values(names)) if (!options[key]) throw new Error(`Missing ${key}; see docs/product-preview/SIGNING.md`);
