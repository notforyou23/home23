...
const result = spawnSync(
  ...
  {
    stdio: "inherit",
    // macOS assigns a long per-user TMPDIR; production UDS fixtures otherwise
    // exceed the platform's 103-byte socket-path limit before exercising code.
    env: process.platform === "darwin" ? { ...process.env, TMPDIR: "/private/tmp" } : process.env,
  },
);
