...
export function productEnvironment(homeRoot, { prepare = false } = {}) {
  ...
  if (prepare) for (const directory of [runtime, userHome, pm2Home, socketRoot]) privateDirectory(directory);
  return {
...
export function validatePortPlan(plan) {
  if (!plan || !PORT_KEYS.every(key => Number.isInteger(plan[key]) && plan[key] >= 20000 && plan[key] <= 60999)
    || new Set(PORT_KEYS.map(key => plan[key])).size !== PORT_KEYS.length) throw new Error('The saved Home23 port plan is invalid.');
  return plan;
}
...
export async function withReservedPorts(plan, callback) {
  validatePortPlan(plan);
  const servers = [];
  try {
    for (const key of PORT_KEYS) servers.push(await reserve(plan[key]));
    return await callback();
  } finally { await Promise.all(servers.map(server => new Promise(resolve => server.close(resolve)))); }
}
export async function choosePortPlan() {
  ...
  for (let attempt = 0; attempt < 500; attempt++) {
    const base = 20000 + ((first - 20000 + attempt * PORT_KEYS.length) % 40000);
    const plan = Object.fromEntries(PORT_KEYS.map((key, index) => [key, base + index]));
    try { return await withReservedPorts(plan, async () => plan); }
