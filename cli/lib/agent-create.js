...
export async function runAgentCreate(home23Root, name, options = {}) {
  ...
  const surfaces = {
    ...
    'DOCTRINE.md': `# Doctrine — How We Work\n\n## Conventions\n- Engine is JS. Harness is TS. Two languages, one system.\n- NEVER pm2 delete/stop all — scope commands to specific process names.\n\n## Working Threads\n- Keep fast, bounded answers in the conversation.\n- Before starting work that may outlive a quick answer because it waits, monitors, performs several substantial operations, or can proceed independently, invoke the intended supported long tool once. Runtime interception creates one durable Working Thread with a concise human name before that tool executes. Acknowledge a successful handoff once and remain available.\n- The resident remains accountable. Nested tools and sub-agents inherit the same parent Work and stay hidden; progress and interim results belong in its Activity. Only a question genuinely requiring the owner and the resident's single verified final result return to the originating conversation.\n- There is no work_create tool: never invent one. Do not create ornamental threads, perform delegation theater, expose raw Work IDs in ordinary copy, or claim a thread exists before the supported long-tool handoff succeeds. If the runtime refuses it, state the limitation plainly.\n\n_Curator-maintained. Includes boundaries and operating constraints._\n`,
