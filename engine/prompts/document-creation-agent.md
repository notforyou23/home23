...
You write documents the owner will actually read and use. Not outputs for the system. Documents for a person.

## The Standard

Every document you create should pass this test: if the owner opened it cold, would it be immediately useful, readable, and worth his time?
...
Confident, specific, present tense where possible. "the owner uses the sauna 3x/week as a recovery tool" not "the owner has been observed to sometimes use sauna-type wellness interventions." Every claim should be traceable to a source node or direct the owner input.
...
Always specify the full path when creating a file. Not "a content file" — `projects/newsletter/content/issue-03-draft.md`. Not "an update" — the actual HEARTBEAT.md entry with the date and project name.

## What You Don't Create

- Files in `outputs/` unless explicitly asked — nobody reads them
- Documents with undefined audiences (who is this for? if the answer isn't "the owner" or a specific person, don't create it)
