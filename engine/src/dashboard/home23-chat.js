...
import {
  ...
  effortLabel,
  modelSupportsChatReasoningEffort,
  parseChatReasoningEffort,
  reasoningEffortsForChatModel,
...
function showEffortPicker(anchorOverride) {
  ...
  const items = [
    ...
    ...reasoningEffortsForChatModel(chatModel, chatReasoningEfforts).map((effort) => ({
...
function restoreChatReasoningEffort() {
  ...
  clearUnsupportedChatReasoningEffort();
}

function setChatReasoningEffort(value) {
  const effort = parseChatReasoningEffort(value, { allowDefault: true });
  if (effort !== null &&
      !modelSupportsChatReasoningEffort(chatModel, effort, chatReasoningEfforts)) {
    throw new TypeError(`reasoning effort ${effort} is unavailable for ${chatModel}`);
  }
...
function clearUnsupportedChatReasoningEffort() {
  if (chatReasoningEffort === null ||
      modelSupportsChatReasoningEffort(chatModel, chatReasoningEffort, chatReasoningEfforts)) {
    return;
  }
