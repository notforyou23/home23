(() => {
  ...
  const effortLabel = Object.freeze({
    none: "None",
    low: "Low",
    medium: "Medium",
    high: "High",
  ...
  function executionEffortOptionsHtml(options, selected) {
    const efforts = Selection.reasoningEffortsForModel(options, selected.modelAlias);
    return `<option value=""${selected.reasoningEffort === null ? " selected" : ""}>Default · ${esc(effortLabel[options.defaultReasoningEffort] || options.defaultReasoningEffort)}</option>${efforts.map((effort) =>
  ...
  function renderConversation(channel, messages, options = {}) {
    ...
    $("composer-model")?.addEventListener("change", () => {
      ...
      const options = state.executionOptions.get(channel.id);
      const effortSelect = $("composer-effort");
      if (options && effortSelect) {
        effortSelect.innerHTML = executionEffortOptionsHtml(
          options,
          executionSelection(channel.id),
        );
      }
    });
