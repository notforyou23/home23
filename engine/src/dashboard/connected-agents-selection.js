(function connectedAgentsSelectionModule(root, factory) {
    ...
    const models = source.models.map((raw) => {
      ...
      if (model.reasoningEfforts !== undefined) {
        if (!Array.isArray(model.reasoningEfforts) || model.reasoningEfforts.length === 0) {
          throw new TypeError("execution model efforts are invalid");
        }
        reasoningEfforts = model.reasoningEfforts.map((value) =>
          effort(value, false, "execution model effort"));
        if (new Set(reasoningEfforts).size !== reasoningEfforts.length) {
          throw new TypeError("execution model efforts must be unique");
        }
      }
      return Object.freeze({
        ...
        reasoningEfforts: reasoningEfforts ? Object.freeze(reasoningEfforts) : null,
      });
    });
  ...
  function reasoningEffortsForModel(options, modelAlias) {
    const model = modelAlias === null
      ? options.models.find((entry) =>
        entry.provider === options.defaultProvider && entry.model === options.defaultModel)
      : options.models.find((entry) => entry.alias === modelAlias);
  ...
  function normalizePreference(value, options) {
    ...
    const normalizedEffort = reasoningEffort !== null &&
      (typeof reasoningEffort !== "string" ||
        !reasoningEffortsForModel(options, modelAlias).includes(reasoningEffort))
      ? null
      : reasoningEffort;
    return Object.freeze({ modelAlias, reasoningEffort: normalizedEffort });
  }
