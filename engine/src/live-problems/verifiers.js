...
const PROBE_LOCK_TIMEOUT_MS = 30_000;
const PROBE_LOCK_STALE_MS = 60_000;
const PROBE_LOCK_RETRY_MS = 10;

function delay(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

async function acquireProbeLock(root, targetPath) {
  const lockRoot = path.join(root, '.home23-create-file-probe-locks');
  const lockName = crypto.createHash('sha256')
    .update(`${root}\0${targetPath}`)
    .digest('hex');
  ...
  while (true) {
    try {
      ...
      fs.writeFileSync(path.join(lockPath, 'owner'), token, { flag: 'wx' });
      return () => {
        try {
      ...
      try {
        ...
        if (Date.now() - stat.mtimeMs > PROBE_LOCK_STALE_MS) {
        ...
        if (!['ENOENT', 'EEXIST', 'ENOTEMPTY'].includes(staleErr.code)) throw staleErr;
      }

      if (Date.now() - startedAt >= PROBE_LOCK_TIMEOUT_MS) {
        throw new Error(`timed out waiting for probe lock: ${targetPath}`);
      }
      await delay(PROBE_LOCK_RETRY_MS);
    }
  }
}
...
const verifiers = {
  ...
  async create_file_tool_probe(args = {}) {
    ...
    try {
      ...
      releaseProbeLock = await acquireProbeLock(root, targetPath);
      ...
      if (releaseProbeLock) releaseProbeLock();
