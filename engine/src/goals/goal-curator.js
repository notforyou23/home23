...
class GoalCurator {
  ...
  async clusterGoals(goals) {
    ...
    const prompt = `
Analyze these goals and cluster them into research campaigns (groups of 3-5 related goals).
...
1. Theme (short name)
2. Description (what this campaign investigates)
3. Goal IDs to include
...
Create 2-4 meaningful campaigns. Skip goals that don't fit well.
    `.trim();
  ...
  async groupRelatedGoals(goals) {
    ...
    const prompt = `
Analyze these mature goals and identify 1-2 synthesis opportunities.

MATURE GOALS:
${goalList}

For each synthesis group, provide:
1. Theme (what connects them)
2. Goal IDs to synthesize
3. Synthesis description (higher-level goal)
