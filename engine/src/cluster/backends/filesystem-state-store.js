...
class FilesystemStateStore {
  ...
  async connect() {
    try {
      ...
      const dirs = [
        ...
        path.join(this.tasksDir, 'assigned'),
  ...
  /**
   * Upsert a task (writes to pending initially)
   */
  async upsertTask(task) {
    try {
      let taskPath;
      
      // Determine location based on state
      if (task.state === 'CLAIMED' || task.state === 'IN_PROGRESS') {
        const instanceId = task.claimedBy || this.instanceId;
        const assignedInstanceDir = path.join(this.tasksDir, 'assigned', instanceId);
        await this.helpers.ensureDirectory(assignedInstanceDir);
        taskPath = path.join(assignedInstanceDir, `${task.id}.json`);
      } else if (task.state === 'DONE') {
        taskPath = path.join(this.tasksDir, 'complete', `${task.id}.json`);
      } else if (task.state === 'FAILED') {
        taskPath = path.join(this.tasksDir, 'failed', `${task.id}.json`);
      } else if (task.state === 'BLOCKED') {
        taskPath = path.join(this.tasksDir, 'blocked', `${task.id}.json`);
      } else {
        taskPath = path.join(this.tasksDir, 'pending', `${task.id}.json`);
      }
  ...
  /**
   * Get a task by ID (searches all task subdirectories)
   */
  async getTask(taskId) {
    try {
      ...
      // Also search assigned subdirectories
      const assignedDir = path.join(this.tasksDir, 'assigned');
      const instances = await this.helpers.listDirectory(assignedDir);
      for (const inst of instances) {
        searchDirs.push(path.join(assignedDir, inst));
      }
      
      for (const dir of searchDirs) {
        const taskPath = path.join(dir, `${taskId}.json`);
        if (await this.helpers.fileExists(taskPath)) {
  ...
  async claimTask(taskId, instanceId, ttlMs) {
    ...
    try {
      ...
      const assignmentPath = path.join(assignedInstanceDir, `${taskId}.json`);

      // Check if already assigned to any instance
      const allInstances = await this.helpers.listDirectory(path.join(this.tasksDir, 'assigned'));
      for (const inst of allInstances) {
        const instDir = path.join(this.tasksDir, 'assigned', inst);
        const taskFile = path.join(instDir, `${taskId}.json`);
        if (await this.helpers.fileExists(taskFile)) {
          // Already assigned
          const assignment = await this.helpers.atomicReadJSON(taskFile, null);
          if (assignment && assignment.claimExpires > Date.now()) {
            return false; // Still claimed by another instance
          }
          // Claim expired - remove stale assignment
          try {
      ...
      const pendingPath = path.join(this.tasksDir, 'pending', `${taskId}.json`);
      const task = await this.helpers.atomicReadJSON(pendingPath, null);

      if (!task) {
        this.logger.warn('[FilesystemStateStore] claimTask: task not found in pending', { taskId });
        return false;
      }
      ...
      // Write to assigned directory
      await this.helpers.atomicWriteJSON(assignmentPath, task);
  ...
  async releaseTask(taskId, instanceId) {
    try {
      ...
      for (const inst of instances) {
        ...
        const instDir = path.join(assignedDir, inst);
        const taskFile = path.join(instDir, `${taskId}.json`);
        
        if (await this.helpers.fileExists(taskFile)) {
          const task = await this.helpers.atomicReadJSON(taskFile, null);
          if (task) {
            // Reset task state
            task.state = 'PENDING';
            task.claimedBy = null;
            ...
            const pendingPath = path.join(this.tasksDir, 'pending', `${taskId}.json`);
            await this.helpers.atomicWriteJSON(pendingPath, task);
  ...
  async startTask(taskId, instanceId) {
    try {
      const assignedInstanceDir = path.join(this.tasksDir, 'assigned', instanceId);
      const taskPath = path.join(assignedInstanceDir, `${taskId}.json`);
      
      const task = await this.helpers.atomicReadJSON(taskPath, null);
      if (!task) {
        this.logger.warn('[FilesystemStateStore] startTask: task not found', { taskId, instanceId });
        return false;
      }
  ...
  async completeTask(taskId) {
    try {
      // Find task in assigned directories
      const assignedDir = path.join(this.tasksDir, 'assigned');
      const instances = await this.helpers.listDirectory(assignedDir);
      ...
      for (const inst of instances) {
        ...
        const taskFile = path.join(instDir, `${taskId}.json`);
        if (await this.helpers.fileExists(taskFile)) {
          task = await this.helpers.atomicReadJSON(taskFile, null);
          sourcePath = taskFile;
          break;
        }
      }
      ...
      const completePath = path.join(this.tasksDir, 'complete', `${taskId}.json`);
      await this.helpers.atomicWriteJSON(completePath, task);
  ...
  async failTask(taskId, reason) {
    try {
      ...
      const failedPath = path.join(this.tasksDir, 'failed', `${taskId}.json`);
      await this.helpers.atomicWriteJSON(failedPath, task);
