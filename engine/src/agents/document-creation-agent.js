...
class DocumentCreationAgent extends BaseAgent {
  ...
  async execute() {
    ...
    if (savedDocument.filePath && path.isAbsolute(savedDocument.filePath)) {
      ...
      // Fallback to standard output directory (legacy case)
      outputDir = this.config.logsDir
        ? path.join(this.config.logsDir, 'outputs', 'document-creation', this.agentId)
        : path.join(process.cwd(), 'runtime', 'outputs', 'document-creation', this.agentId);
    }
  ...
  determineFormat(text) {
    if (text.includes('.json') || text.includes('json artifact') || text.includes('valid json')) {
      return 'json';
    }
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: 'You are creating a document using COSMO\'s accumulated knowledge. Use ONLY the specific facts and findings provided. Be detailed, substantive, and grounded in the research. NO generic corporate language.'
          },
          {
            role: 'user',
            content: prompt
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: `You are an expert document writer creating a ${spec.type} for a ${spec.audience} audience. 
      ...
      try {
        ...
        const response = await this.callGPT5({
          messages: [
            {
              role: 'system',
              content: 'You are COSMO in full creative mode. You have access to knowledge across all domains. Be brilliant, original, and fearless in your synthesis. This is NOT a corporate document - create something truly remarkable.'
            },
            {
              role: 'user',
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: 'You are evaluating creative outputs from COSMO. Your job is to either SELECT the most brilliant branch OR SYNTHESIZE the best elements from multiple branches into something even better.'
          },
          {
            role: 'user',
...
Your response:`
  ...
  async saveDocument(document, spec) {
    ...
    let content = document.content || document.formattedContent;
    ...
    // Honor exact-path contracts even when meta-coordinator forgot mission.deliverable.
    let deliverableSpec = this.mission?.deliverable || null;
    if (!deliverableSpec?.filename) {
      const pathCandidates = [
        ...extractFileDeliverablesFromGoal({
          description: this.mission?.description,
          doneWhen: this.mission?.doneWhen,
          reason: this.mission?.rationale,
          metadata: this.mission?.metadata,
        }),
        ...extractPathsFromText(
          [
            this.mission?.description,
            ...(this.mission?.successCriteria || []),
            this.mission?.rationale,
          ]
            .filter(Boolean)
            .join('\n')
        ),
      ...
      if (pathCandidates[0]) {
        deliverableSpec = buildDeliverableSpecFromPath(pathCandidates[0]);
        this.mission.deliverable = deliverableSpec;
        this.logger?.info?.('📎 Inferred exact-path deliverable from mission text', {
          agentId: this.agentId,
    ...
    if (deliverableSpec?.filename) {
      content = coerceJsonFileContent(content, deliverableSpec.filename);
      if (deliverableSpec.format === 'json' || deliverableSpec.filename.endsWith('.json')) {
        spec.format = 'json';
      }
    }
    ...
    const runOutputDir = this.config.logsDir
      ? path.join(this.config.logsDir, 'outputs', 'document-creation', this.agentId)
      : path.join(process.cwd(), 'runtime', 'outputs', 'document-creation', this.agentId);
    ...
    if (deliverableSpec && !this.pathResolver) {
      const deliverableDir = path.join(
        process.cwd(),
        deliverableSpec.location || 'runtime/outputs/'
      );
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: `You are an expert technical writer creating code documentation for a ${spec.audience} audience.
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: `You are an expert data analyst creating spreadsheet analysis reports for a ${spec.audience} audience.
...
`;
  ...
  /**
   * Exact-path contracts: filesCreated alone is not enough if mission demanded
   * a specific top-level outputs file that never landed.
   */
  assessAccomplishment(executeResult, results) {
    const base = super.assessAccomplishment(executeResult, results);
    if (!base.accomplished) return base;

    const deliverableFilename =
      this.mission?.deliverable?.filename ||
      extractFileDeliverablesFromGoal({
        description: this.mission?.description,
        doneWhen: this.mission?.doneWhen,
        reason: this.mission?.rationale,
    ...
    if (!deliverableFilename) return base;

    const rel = normalizeOutputsRelativePath(deliverableFilename) || deliverableFilename;
    const outputsRoot =
      this.config?.outputsDir ||
      (this.config?.logsDir ? path.join(this.config.logsDir, 'outputs') : null) ||
      path.join(process.cwd(), 'runtime', 'outputs');
    const expectedPath = path.join(outputsRoot, rel);

    // Also accept absolute path recorded on deliverable result
    const deliverableResults = (results || []).filter((r) => r && r.type === 'deliverable' && r.path);
    const matchedDeliverable = deliverableResults.some((r) => {
      const p = String(r.path || '').replace(/\\/g, '/');
      return p.endsWith('/' + rel) || p.endsWith(rel) || path.basename(p) === path.basename(rel);
    });

    let existsOnDisk = false;
    try {
      existsOnDisk = fsSync.existsSync(expectedPath);
    } catch {
      existsOnDisk = false;
    }

    if (existsOnDisk || matchedDeliverable) {
      return {
        ...base,
        metrics: {
          ...base.metrics,
          exactPathDeliverable: rel,
          exactPathSatisfied: true,
        },
      };
    }

    // Wrote something agent-scoped but missed the exact contract path.
    return {
      accomplished: false,
      reason: `Exact-path deliverable missing: expected ${rel} under outputs/ (agent-scoped write is not enough)`,
      metrics: {
        ...base.metrics,
        exactPathDeliverable: rel,
