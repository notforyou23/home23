...
class AgentExecutor {
  ...
  async spawnAgent(missionSpec) {
    ...
    try {
      ...
      // OPTIONAL: Load external prompt + system context from prompts/ directory
      // Graceful failure is critical: prompt files are optional and must never block spawning.
      try {
        const promptsDir = path.join(__dirname, '../../prompts');
        const agentType = missionSpec.agentType;
        const agentPromptPath = path.join(promptsDir, `${agentType}.md`);
        const systemContextPath = path.join(promptsDir, 'system-context.md');
        ...
        if (fs.existsSync(agentPromptPath)) {
          ...
          if (content && !content.includes('<!-- Filled by prompt-writer -->')) {
            externalPrompt = content;
          }
        }

        if (fs.existsSync(systemContextPath)) {
          const content = fs.readFileSync(systemContextPath, 'utf8');
          if (content && !content.includes('<!-- Filled by prompt-writer -->')) {
            systemContext = content;
          }
        }

        if (externalPrompt || systemContext) {
          missionSpec = { ...missionSpec, externalPrompt, systemContext };
        }
      } catch (_) {
        // Non-fatal: prompt injection is optional
      }
      ...
      if (this.evaluation) {
        this.evaluation.trackAgentSpawned(agent.agentId, missionSpec.agentType, missionSpec.goalId);
      }

      // Emit real-time agent spawned event
  ...
  async integrateResults(agentResults, qaMetadata = null) {
    ...
    if (mission.goalId && status === 'completed' && !isReviewPipeline) {
      try {
        ...
        if (normalizedType === 'code-creation') {
          const outputDir = `runtime/outputs/code-creation/${agentId}`;
          try {
    ...
    await this.createFollowUpGoals(results, agentId);
  ...
  getDefaultAgentOutputDir(agentId, agentType) {
    ...
    return path.join(process.cwd(), 'runtime', 'outputs', agentDir, String(agentId));
  }
  ...
  async registerTaskArtifactsFromAgentRun(agentResults) {
    ...
    for (const dir of dirs) {
      if (!(await this.safePathExists(fs, dir))) continue;
      ...
      if (deliverablesManifest && Array.isArray(deliverablesManifest.deliverables)) {
        for (const f of deliverablesManifest.deliverables) {
          artifacts.push({
            path: path.join(path.relative(path.join(process.cwd(), 'runtime', 'outputs'), dir), f.path || ''),
            absolutePath: f.absolutePath || path.join(dir, f.path || ''),
            size: f.size || null,
      ...
      if (manifest && Array.isArray(manifest.files)) {
        for (const f of manifest.files) {
          artifacts.push({
            path: path.join(path.relative(path.join(process.cwd(), 'runtime', 'outputs'), dir), f.path || ''),
            absolutePath: path.join(dir, f.path || ''),
            size: f.size || null,
            checksum: f.checksum || null,
            agentId,
            agentType: this.normalizeAgentTypeDir(agentType),
      ...
      for (const name of names) {
        ...
        try {
          ...
          artifacts.push({
            path: path.join(path.relative(path.join(process.cwd(), 'runtime', 'outputs'), dir), name),
            absolutePath: abs,
            size: st.size,
            checksum: null,
            agentId,
   ...
   */
  async createFollowUpGoals(results, agentId) {
    ...
    for (const direction of directionsToCreate) {
      if (typeof direction === 'string' && direction.length > 20) {
        try {
          const newGoalId = await this.goals.addGoal({
            description: direction,
            priority: 0.5, // Medium priority
            source: `follow_up_from_${agentId}`,
            parentAgent: agentId,
            created: new Date()
          });
  ...
  async enrichMissionWithArtifacts(mission) {
    try {
      ...
      const artifactAwareTypes = [
        ...
        'synthesis',
        'quality_assurance',
        'research'  // NEW: Research can use predecessor research outputs
      ];
      ...
      // Gather artifacts from predecessors
      const artifacts = await this.gatherPredecessorArtifacts(mission);
  ...
  buildArtifactContext(artifacts) {
    ...
    for (const [sourceAgentId, files] of Object.entries(bySource)) {
      ...
      for (const file of files) {
        ...
        context += `- \`${file.relativePath}\`${size}\n`;
      }
      context += '\n';
    }
    
    context += 'These files are already uploaded to your execution environment and can be read, imported, or processed directly.\n';
