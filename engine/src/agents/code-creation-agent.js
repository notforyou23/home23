...
2. For each file, inside the Python tool call:
   ...
   - Build the full ${spec.language} source text as a string variable named \`source_text\` (no placeholders or TODOs).
    ...
    try {
      const response = await this.callGPT5({
        messages: [
          {
            role: 'system',
            content: `You are an expert ${spec.language} developer creating functional, well-structured code.
  ...
  async saveCodeFiles(codePackage, spec) {
    ...
    if (this.capabilities) {
      await this.capabilities.writeFile(
        path.relative(process.cwd(), mainFilePath),
    ...
    if (this.capabilities) {
      await this.capabilities.writeFile(
        path.relative(process.cwd(), structurePath),
...
`;
  ...
  async validateExportedFiles() {
    ...
    const findingText = `File validation complete: ${validatedFiles.length} valid, ${invalidFiles.length} invalid.` +
      (invalidFiles.length > 0 
        ? ` Failures: ${invalidFiles.map(f => `${f.path} (${f.reason})`).join(', ')}`
        : '');
    
    await this.addFinding(findingText, 'code_validation');
    ...
    if (successRate >= 0.8) {
      ...
      await this.addFinding(
        `Code validation below threshold: ${validFiles}/${totalFiles} files valid (${(successRate * 100).toFixed(1)}%)`,
        'quality_warning'
      );
    }
  }
