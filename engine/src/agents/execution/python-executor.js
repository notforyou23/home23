...
class PythonExecutor {
  constructor(sandbox, logger, config = {}) {
    ...
    this.workingDir = path.resolve(config.workingDir || 'runtime/outputs');
  ...
  async execute(code, options = {}) {
    ...
    try {
      ...
      this.logger?.info('File tracking', {
        outputDir,
      ...
      this.logger?.info('New files detected', {
      ...
      // Still try to capture any files that were created
      try {
        const afterFiles = await this.scanDirectory(outputDir);
        const newFiles = this.findNewFiles(beforeFiles, afterFiles);
        
        return this.formatErrorResults(error, newFiles);
      } catch (scanError) {
        // If scanning fails, return error without files
        return this.formatErrorResults(error, []);
      }
    } finally {
      // Cleanup temp script (but keep generated files)
