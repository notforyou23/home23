...
class BaseAgent extends EventEmitter {
  ...
  async onComplete() {
    ...
    this.logger.debug('Agent completed', { agentId: this.agentId }, 3);
  }
  ...
  async appendToJournal(entry) {
    ...
    if (this.capabilities && this.capabilities.appendFile) {
      try {
        const logicalPath = path.relative(process.cwd(), journalPath);
  ...
  async discoverFiles(options = {}) {
    const {
      agentTypes = ['code_creation', 'code_execution', 'document_creation', 'document_analysis'],
      fileType = null,
      sourceAgentId = null,
      limit = 20,
      maxAgeMs = null,
    ...
    const TAG_MAP = {
      ...
      'research': ['research_output_files']  // NEW: Research corpus files
    };
          ...
          for (const item of taggedNodes) {
            ...
            try {
              ...
              if (data?.files && Array.isArray(data.files)) {
                // Format: { agentId, files: [{filename, relativePath, size}] }
                files = data.files;
  ...
  async readFileViaMCP(filePath, maxSizeBytes = 100 * 1024 * 1024) {
    ...
    try {
      ...
      if (!result?.content?.[0]) {
        throw new Error(`Empty response from MCP server for: ${filePath}`);
      }
      ...
      if (data.size > maxSizeBytes) {
        throw new Error(
          `File too large: ${filePath} is ${(data.size / (1024 * 1024)).toFixed(2)}MB ` +
  ...
  async uploadPredecessorArtifacts(artifacts) {
    ...
    if (results.uploaded > 0 || results.failed > 0) {
      ...
      // Add insight about artifact availability
      if (results.uploaded > 0) {
        await this.addInsight(
          `Successfully uploaded ${results.uploaded} file(s) from predecessor agents. ` +
          `Files available: ${results.uploadedFiles.map(f => f.filename).join(', ')}`,
          'artifact_upload'
        );
      }
    }
