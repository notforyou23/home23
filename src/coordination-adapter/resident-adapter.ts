...
export const RESIDENT_TURN_EVIDENCE_TAXONOMY: CoordinationExecutionEvidenceTaxonomy = Object.freeze({
  ...
  switch (durable.event.type) {
    ...
    case 'cache':
      return { ...common, read: durable.event.read, write: durable.event.write,
        ...
        inputTotal: durable.event.inputTotal ?? null,
        provider: durable.event.provider ?? null, model: durable.event.model ?? null };
    case 'status':
...
/**
 * Shared lossless projector for resident AgentLoop evidence.  Alternate
 * resident execution paths must use this rather than inventing a second
 * Thoughts/Tools taxonomy.
 */
export class ResidentCommunicationEventProjector {
  private readonly parentEvents = new Map<string, string>();

  constructor(
    private readonly context: ResidentCommunicationContext,
    private readonly origin: CoordinationTurnOrigin,
    private readonly taxonomy: CoordinationExecutionEvidenceTaxonomy =
      RESIDENT_TURN_EVIDENCE_TAXONOMY,
  ) {}

  project(durable: ResidentDurableEvent): CommunicationEventInput {
...
export class ResidentCoordinationAdapter {
  ...
  async recoverCompleted(request: ResidentWorkRequest): Promise<ResidentRun> {
    ...
    const started = await this.agent.runWithTurn(request.chatId, request.instruction, {
      ...
      plannedExecution: request.plannedExecution,
      ...
      onEvent: (durable) => {
        ...
        const eventId = durableCommunicationEventId(durable, this.evidenceTaxonomy);
        if (!this.communications || !request.communication) return;
        replayChain = replayChain.then(async () => {
          // Keep completed journal replay fair to HTTP and fence changes too.
  ...
  private async run(
    ...
    const fenceCallback = (callback: () => void | Promise<void>): void => {
      callbackChain = callbackChain.then(async () => {
        // Let HTTP, cancellation, and heartbeats run between durable event writes.
        await new Promise<void>(resolve => setImmediate(resolve));
      ...
      callbackChain.catch(() => {
        ...
        if (active) void Promise.resolve().then(() => this.agent.stop(active.chatId, active.turnId)).catch(() => undefined);
      });
    };
    ...
    const started = await this.agent.runWithTurn(request.chatId, request.instruction, {
      ...
      onDurableStart: async ({ turnId, persistedAt, selection }) => {
        ...
        if (this.coordination.heartbeat) {
          await this.coordination.heartbeat(binding);
          heartbeatTimer = setInterval(() => {
            if (this.active.get(origin.workId)?.cancelling) return;
            void Promise.resolve().then(() => this.coordination.heartbeat!(binding)).catch(() => {
    ...
    const receipt = (async (): Promise<ResidentTerminalReceipt> => {
      try {
        ...
        if (terminal.status === 'cancelled') {
          if (request.plannedExecution && durableTerminal === null) throw new Error('planned cancellation is awaiting durable executor stop evidence');
  ...
  /** Stop only the exact execution whose canonical lease was already revoked. */
  async stopRevoked(binding: ResidentLeaseBinding): Promise<boolean> {
    const active = this.active.get(binding.workId);
    if (!active || active.binding.attemptId !== binding.attemptId ||
        active.binding.leaseId !== binding.leaseId ||
        active.binding.fencingToken !== binding.fencingToken ||
        active.binding.holderInstanceId !== binding.holderInstanceId ||
        active.binding.holderPrincipalId !== binding.holderPrincipalId) return false;
    if (!(await this.coordination.cancellationState?.(binding))) {
      throw new Error('resident stop requires the exact revoked lease');
    }
    active.cancelling = true;
    return (await this.agent.stop(active.chatId, active.turnId)).stopped;
  }
