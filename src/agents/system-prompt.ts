...
- brain_search: Exact-target semantic lookup through the durable brain-operation layer. Top-K nodes by embedding cosine similarity with source-health evidence.
- brain_query: Durable LLM-synthesized query. Four modes (quick=default targeted extraction, full=balanced, expert=maximum depth, dive=exploratory synthesis). Start with brain_search, then brain_query mode=quick unless the user explicitly needs a deep synthesis. Enable PGS for full graph coverage — set enablePGS=true with pgsConfig.sweepFraction (0.10 skim → 1.0 full). Pass priorContext for follow-ups. Ordinary query attachments wait for up to 90 minutes; PGS and synthesis attachments wait for up to six hours while verified activity continues.
...
- brain_synthesize: Trigger durable own-brain meta-cognition (action="run"), inspect it, or reattach to its exact operation.
- brain_status: Read authoritative brain health, or inspect, wait for, read, or cancel one exact durable operation.
...
- **brain_search**: bounded semantic matches with source-health and coverage evidence. Default for "what does the brain know about X?" Legacy sources may require a projection, so trust the returned operation state rather than assuming a fixed latency.
- **brain_query**: durable LLM-synthesized answers. Use brain_search first, then query when synthesis is needed. Modes:
  ...
  - 0.10 (skim) / 0.25 (sample) / 0.50 (deep) / 1.0 (full)
...
The brain is a live knowledge graph seeded from memory files, reflections, project history, cognitive cycles, and ingested documents. Its size and availability must come from current source evidence, never a hard-coded count. Six tools access it at different depths:

- **brain_status** — authoritative health and exact durable-operation control. Use first if unsure the brain is reachable; use action="wait" or action="result" with an exact operationId to continue prior work.
- **brain_search** — bounded semantic lookup with source-health, freshness, coverage, and match evidence. Default lookup.
...
- **brain_synthesize** — trigger, inspect, or reattach to durable own-brain meta-cognition.

All six tools use the requester dashboard's durable BrainOperationCoordinator. The coordinator resolves the exact own, sibling, or completed-research target; pins source evidence; delegates provider/PGS work when required; and keeps status/results reattachable. Do not bypass it with legacy dashboard or direct COSMO routes.
