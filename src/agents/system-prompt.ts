...
Ordinary query attachments wait for up to 90 minutes. PGS launches detached immediately; an explicitly requested PGS or synthesis reattachment can wait for up to six hours. Verified operation activity renews the turn lease, but visible status is coalesced. Judge provider liveness from lastProviderActivityAt; lastProgressAt records committed batch progress and may legitimately lag during a long sweep. A transport disconnect, attachment deadline, or Chat Stop detaches durable work without cancelling it. Preserve the returned operation ID and use brain_status status/result instead of claiming failure, restarting the operation, or guessing from stale telemetry. Only brain_status action:"cancel" for the exact operation ID cancels durable work.
