...
export interface M14AuthorityInput {
  databasePath: string;
...
export function executeM14AuthorityTransition(input: M14AuthorityInput) {
  const database = openCoordinationDatabase({
    path: input.databasePath,
    applicationVersion: "home23-coordination-m14-authority",
  });
  try {
    const history = database.readAll<AuthorityEpoch>(
      `SELECT capability, epoch, mode, writer,
       ...
       WHERE capability = 'messages'
       ORDER BY epoch`,
    );
    if (history.length === 0) {
      throw new Error("messages authority history is missing");
    }
    const current = history.at(-1)!;
    const proposed: AuthorityEpoch = {
      capability: "messages",
    ...
    if (
      ...
      proposed.writer !== COORDINATION_MESSAGES_WRITER
    ) {
      throw new Error(
        `M14 canonical writer must be exactly ${COORDINATION_MESSAGES_WRITER}`,
      );
    }

    const expectedFlags = Object.keys(FEATURE_FLAG_REGISTRY);
    const enabledFlags = expectedFlags.filter(
      (key) => input.receipt.activeFlags[key] === true,
    );
    if (
      expectedFlags.some(
        (key) => typeof input.receipt.activeFlags[key] !== "boolean",
      )
    ) {
      throw new Error(
        "authority receipt must contain every registered Boolean flag",
      );
    }
    const required = proposed.mode === "canonical"
      ? [
      ...
      : [];
    if (
      enabledFlags.length !== required.length ||
      required.some((flag) => !enabledFlags.includes(flag))
    ) {
      throw new Error(
        proposed.mode === "canonical"
          ? "M14 authority receipt must enable exactly process, public API, and Jerry"
          : "shadow/rollback authority receipt must remain feature-off",
      );
    }
    ...
    if (validation.decision !== "valid") {
      throw new Error(`M14 authority transition denied: ${validation.reason}`);
    }
    const plan = {
      mode: input.apply === true ? "apply" : "preflight",
      current,
      proposed,
    ...
    if (input.liveAuthorized !== true) {
      throw new Error(
        "M14 authority apply requires --live-authorized and a valid signed receipt",
      );
    }
    ...
    database.mutateWithEvent((transaction) => {
      ...
      return {
        ...
        event: {
          ...
          aggregateId: "authority:messages",
