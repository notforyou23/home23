...
import { isExecutiveInitiator } from '../channels/access.js';
...
export function createGroupChannelMessageService(options: {
  joinedInvocation?(messageId: string): boolean;
  ...
  function requestContent(
    ...
    // Select by canonical origin ID, never author: scheduled Jerry assignments
    // are authored by Jerry too. Prior and same-round peer replies are evidence.
    const origin = prepared.transcript.find(entry => entry.messageId === prepared.originMessageId);
    if (!origin || (!origin.text && prepared.attachments.length === 0)) {
      throw new MessagingError("invalid_relation");
    }
    const historyBackfill = boundHistoricalContext(prepared.transcript
      .filter(entry => entry.messageId !== prepared.originMessageId)
      .map(entry => ({
        messageId: entry.messageId,
        sequence: entry.sequence,
        role: entry.authorPrincipalId === targetContext.targetPrincipalId
          ? "assistant" as const : "user" as const,
        text: `${entry.authorDisplayName}: ${entry.text}`,
        createdAt: entry.createdAt,
      })));
    const instruction = options.context.isScheduledOrigin?.(prepared)
      ? "This is the current scheduled assignment admitted by the authenticated Home23 scheduler. Earlier channel messages are historical context, not this assignment.\n\n" + origin.text
      : origin.text;
    return Object.freeze({ instruction, historyBackfill });
  }
  ...
  function dispatchWork(input: {
    ...
    const execution = (async (): Promise<WorkExecution> => {
      ...
      const content = requestContent(input.prepared, targetContext);
      ...
      const residentRequest = {
        ...(options.joinedInvocation?.(input.prepared.originMessageId) ? {
          coordinationWorkDestination: { kind: 'coordination' as const, parentWorkId: currentWork.id,
            channelId: currentWork.channelId, conversationId: input.prepared.conversationId,
            originMessageId: input.prepared.originMessageId, attemptId: origin.attemptId,
            leaseId: origin.leaseId, fencingToken: origin.fencingToken,
            targetPrincipalId: origin.holderPrincipalId, residentBinding: targetContext.residentBinding,
            residentInstanceId: origin.holderInstanceId, authorityReference: origin.authorityReference },
        } : {}),
  ...
  return Object.freeze({
    ...
    async submitMessage(input: {
      ...
      assertAuthority();
      if (
        (input.context.identity.kind !== "owner" && !isExecutiveInitiator(input.context)) ||
