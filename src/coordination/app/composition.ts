...
export function createCoordinationProcess(
  ...
  return Object.freeze({
    start: async () => {
      ...
      let projectedEventSequence: number | null = null;
      const projectResidentWorkSnapshot = () => {
        if (residentProjectionRunning) return;
        residentProjectionRunning = true;
        void (async () => {
          if (projectedEventSequence !== null) {
            const changes = await residentWorkProjectionChangesSince(database, projectedEventSequence);
            if (!changes.changed) {
              projectedEventSequence = changes.cursor;
              return;
            }
          }
          // Capture before reading the snapshot. Anything committed during
