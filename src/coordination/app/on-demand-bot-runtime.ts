...
class OnDemandBotAgentPort implements ResidentAgentPort {
  ...
  private resolvedSelection(requested: ResidentTurnSelectionRequest): {
    ...
    if (!modelSupportsReasoningEffort(source.model, source.reasoningEffort)) {
      throw new Error("requested on-demand Bot reasoning effort is unavailable for the selected model");
    }
    return {
      source,
    ...
    async resolve(descriptor: DirectMessageTargetDescriptor): Promise<DirectMessageExecutionTarget | undefined> {
      ...
      const target: DirectMessageExecutionTarget = Object.freeze({
        execution: Object.freeze({
          stopRevoked: (binding: Parameters<ResidentCoordinationAdapter["stopRevoked"]>[0]) => requireExecutionRuntime().adapter.stopRevoked(binding),
