...
implements GroupChannelMessageContextPort {
  ...
  isScheduledOrigin(prepared: GroupChannelPreparedContext): boolean {
    ...
    if (!origin || origin.authorPrincipalId !== prepared.actorPrincipalId) return false;
    // Only the coordinator-owned journal establishes scheduled provenance.
    return !!this.database.readOne(
      `SELECT sequence FROM events
       WHERE aggregate_kind = 'scheduled_channel_run' AND aggregate_version = 1
         AND channel_id = ? AND actor_principal_id = ?
         AND json_extract(payload_json, '$.messageId') = ?
         AND json_extract(payload_json, '$.channelId') = ?
         AND json_extract(payload_json, '$.botId') = ?
         AND json_extract(payload_json, '$.prompt') = ?`,
      prepared.channelId, prepared.actorPrincipalId, prepared.originMessageId,
      prepared.channelId, prepared.actorPrincipalId, origin.text,
    );
  }
  ...
  private async preparedFromAdmissionPlan(
    ...
    const instruction = rows.find(message => message.id === plan.originMessageId)?.text ?? "";
  ...
  private async snapshot(input: {
    ...
    if (
      (!(input.context.identity.kind === "owner" && input.context.principalId === "user_owner" && input.originMessage.author.kind === "owner") &&
        !(isExecutiveInitiator(input.context) && input.originMessage.author.kind === "bot")) ||
    ...
    const instruction = boundedMessages.find(message => message.id === input.originMessage.id)?.text ?? "";
  ...
  async prepareSequentialTurn(
    ...
    const instruction = rows.find(message => message.id === input.plan.originMessageId)?.text ?? "";
  ...
  async recover(work: WorkRecord): Promise<GroupChannelPreparedContext> {
    if (
      !["channel.bot_turn", "resident_work_thread"].includes(work.kind) ||
