/** Accepts general files while retaining immutable content, ownership and link guards. */
export const ARTIFACT_GENERAL_FILES_MIGRATION_SQL = `
PRAGMA defer_foreign_keys = ON;

CREATE TABLE artifacts_v16 (
  ...
  declared_content_type TEXT CHECK (
    declared_content_type IS NULL OR (
      length(declared_content_type) BETWEEN 3 AND 255 AND instr(declared_content_type, '/') > 1 AND
      instr(declared_content_type, char(10)) = 0 AND instr(declared_content_type, char(13)) = 0
    )
  ),
  detected_content_type TEXT CHECK (
    detected_content_type IS NULL OR (
      length(detected_content_type) BETWEEN 3 AND 255 AND instr(detected_content_type, '/') > 1 AND
      instr(detected_content_type, char(10)) = 0 AND instr(detected_content_type, char(13)) = 0
    )
  ),
...
CREATE TABLE message_artifacts_v16 (
  message_id TEXT NOT NULL,
  channel_id TEXT NOT NULL,
  artifact_id TEXT NOT NULL REFERENCES artifacts_v16(id),
...
CREATE TABLE attachment_create_idempotency_v16 (
  principal_id TEXT NOT NULL REFERENCES principals(id),
  key_digest TEXT NOT NULL CHECK (length(key_digest) = 64),
  request_digest TEXT NOT NULL CHECK (length(request_digest) = 64),
  artifact_id TEXT NOT NULL UNIQUE REFERENCES artifacts_v16(id),
  created_at TEXT NOT NULL CHECK (length(created_at) = 24),
  PRIMARY KEY (principal_id, key_digest)
) STRICT;

INSERT INTO artifacts_v16 (
...
INSERT INTO message_artifacts_v16 (
...
INSERT INTO attachment_create_idempotency_v16 (
...
ALTER TABLE artifacts_v16 RENAME TO artifacts;
ALTER TABLE message_artifacts_v16 RENAME TO message_artifacts;
ALTER TABLE attachment_create_idempotency_v16 RENAME TO attachment_create_idempotency;
