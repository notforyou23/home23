...
import { isChannelManager } from '../channels/access.js';
...
export class SqlitePersistentMailboxBinder implements PersistentMailboxBinder {
  ...
  async bindDurableBot(input: Parameters<PersistentMailboxBinder["bindDurableBot"]>[0]) {
    assertLifecycleActor(this.database, input);
    ...
    const result = this.database.mutateWithEvent<BotRow>((transaction) => {
      assertLifecycleActor(transaction, input);
      ...
      events.push(receiptEvent(storedReceipt.receipt, storedReceipt.keyDigest, input.requestId, input.actorPrincipalId));
      return {
        value: row,
    ...
    const updated = this.database.mutateWithEvent<BotRow>((transaction) => {
      ...
      return {
        ...
        events: [{
          ...
          createdAt: input.changedAt,
        }, receiptEvent(storedReceipt.receipt, storedReceipt.keyDigest, input.requestId, input.actorPrincipalId)],
      };
    }).value;
