...
import { mountHomeUpdateRoutes } from "../home-update/routes.js";
...
export function createCoordinationRouter(input: {
  ...
  mountHomeUpdateRoutes(router, application);
  ...
  router.get("/api/v1/messages/:messageId", productRead, asyncRoute(async (request, response) => {
    if (!application.capabilities().capabilities.messagesRead || !application.services.messages) throw unavailable("messagesRead");
    const message = await application.services.messages.getMessage({
      context: requireCoordinationContext(response),
      messageId: pathParameter(request.params.messageId),
    });
    if (!message) throw new MessagingError("unknown_message");
  ...
  router.patch("/api/v1/channels/:channelId", messageSend, requireIdempotencyKey(application), jsonBody, asyncRoute(async (request, response) => {
    if (!application.capabilities().capabilities.channelMutation || !application.services.channels) throw unavailable("channelMutation");
    const body = jsonObjectBody(request.body);
    const ifMatch = request.get("If-Match");
    if (!ifMatch || !/^[1-9][0-9]*$/.test(ifMatch) || !Number.isSafeInteger(Number(ifMatch))) {
      throw new CoordinationHttpError("request_invalid", 400, false);
    }
    const result = await application.services.channels.updateChannel({
      context: requireCoordinationContext(response),
      channelId: pathParameter(request.params.channelId),
      expectedVersion: Number(ifMatch),
      title: body.title as string,
      purpose: body.purpose as string,
      memberBotIds: body.memberBotIds as string[],
      responderPolicy: body.responderPolicy as any,
  ...
  router.get("/api/v1/work", productRead, asyncRoute(async (request, response) => {
    if (!application.capabilities().capabilities.work || !application.services.workControl) throw unavailable("work");
    const conversationId = nullableQuery(request.query.conversationId);
  ...
  for (const operation of ["cancel", "retry"] as const) router.post(
    ...
    asyncRoute(async (request, response) => {
      ...
      const result = await application.services.workControl[operation]({ context: requireCoordinationContext(response), workId: pathParameter(request.params.workId), idempotencyKey: coordinationIdempotencyKey(response) });
