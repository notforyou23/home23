...
## External review addendum (2026-08-02)

An independent read-only review of the full tree confirmed revision 2 "genuinely resolves the prior ownership collisions, sleep-proof ambiguity, schema duplication, and lockfile-order problems" and that the plan set preserves COSMO's identity. It raised three planning-residue items, closed at `b9be641d`: `canonicalJsonBytes` now implemented in A Task 3 as an explicit alias of the existing `canonicalBytes`; G's heritage import gains its `.js` ESM extension; the `notApplicableReasons` rejection semantics verified already present (G:3191). Its five P1/P2 "must resolve" items were ground-truthed and are **not standalone-plan defects** — they are in-flight home23/cosmo23 worktree work owned by a concurrent stream (MCP identity unwired, merge-engine IO contract unmet at HEAD, self-reported output truth, eight unregistered test suites, two genuinely missing builtin catalog models — the review's four-missing claim was half right). All are real; all are chipped as separate tasks (7 chips, 2026-08-02) rather than edited from this session, per the worktree-preservation doctrine. They bear on the cosmo23 maintenance-freeze decision (ledger G-3) and G's migration surface, not on the §6 planning gate.
