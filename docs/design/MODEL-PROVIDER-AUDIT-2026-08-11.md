...
1. **Plaintext provider keys sit in a feeder-watched tree and are ingestible into the brain.**
   ...
   **STATUS: PARTIALLY FIXED — AND THE ORIGINAL ASSESSMENT WAS TOO NARROW.** (a) exclusions added incl. `**/config.yaml`, live since the 2026-08-11 restart; (b) Patch 73 (`8d40627c`): the generator writes no keys; 31 run configs scrubbed to `""`.

   **(c) CORRECTION — the first sweep's "zero ingestions" was a FALSE NEGATIVE.** Its shell loop (`grep -c … || echo 0`) made every file report a miss; the all-clear was an artifact of the broken check, not evidence. Re-verified post-restart by testing brain content against the *actual current values* in `config/secrets.yaml`: **four live credentials are present in jerry's live brain base** (`memory-nodes.base-5778129.jsonl.gz`, 24 lines, ~211 candidate nodes) — `providers.ollama-cloud.apiKey`, `providers.brave.apiKey`, `skills.x.apiKey`, and `agents.jerry.discord.token`. Forrest: none. (A parallel `sk-[A-Za-z0-9_-]{20,}` scan returned 565 jerry / 95 forrest hits that are almost entirely **false positives** — hyphenated English like "risk-free-sample", "task-assessment". The equality-against-live-secrets test is the reliable one; pattern scans in both directions were misleading.)

   **The leak is not one lane, it is three.** Node provenance shows credentials entering via `research_runs` (the cosmo23 run configs — the only lane this pass closed), **`projects`** (project docs containing literal env dumps, e.g. a markdown table with `BRAVE_API_KEY="…"`), and **`conversation_sessions`** (compiled transcripts that quoted secrets). Path exclusions cannot fix lanes 2 and 3 — **the real fix is a credential scrubber at the feeder's ingest boundary** (reject/redact key-shaped material before compilation, for every watch path), which does not exist and is now the top open item.

   **(d) OPEN — jtr: rotate all four credentials above** (mandatory, not advisory; they are queryable/exportable from the live brain). **(e) OPEN — node purge:** the existing nodes still carry the values; removal touches brain persistence and needs jtr's call plus its own ceremony. **(f) OPEN — build the feeder-side scrubber.**
