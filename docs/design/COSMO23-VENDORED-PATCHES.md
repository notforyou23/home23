...
**What changed (server API surface):** `server/lib/run-sentinel.js` (new) monitors the active run's `<runDir>/.heartbeat` progress signal every `sentinel.checkIntervalMs` (60s). Wedge = process online with no cycle progress for `sentinel.wedgeThresholdMs` (15min). Bounded ladder (`sentinel.maxAttempts`, default 2): stop children via ProcessManager.stopAll(), then relaunch DIRECTLY from the tracked run directory — `createContinuationRelauncher` replays `<runDir>/metadata.json` through `launchPreparedResearch` with a brain object built from the run path, mirroring `launchResearch`'s 409/isLaunching guards but bypassing catalog resolution entirely (2026-07-22 live wedge drill: catalog-backed `launchResearch` fails young mid-run brains with "Brain not found" — the catalog only lists completed/queryable brains per Patch 69); state persists in `<runDir>/.sentinel.json`. After exhaustion it escalates. Status contract gains ADDITIVE fields only (Patch 9 compat): `health.wedged` (boolean), `health.sentinel` (attempts/maxAttempts/lastReason/escalated/escalatedAt/...), plus flattened `wedged` on `/api/status`. `lifecycle` values are unchanged. Guards: no action while `isLaunching`, within 5min of launch (`sentinel.launchGraceMs`), or when no heartbeat file exists. Watchdog hand-off (Fix 2.2): the sentinel keys ONLY on live signals (process death, progress staleness) — never on the persisted `restartRequested` flag in `.watchdog.json` (it survives boot until the first successful cycle), and no exit-code special-casing (exit 86 = any other death). Heartbeat phases `breaker_cooloff`/`revive_probe` are not counted as wedge time (the engine is managing its own cooloff); belt-and-braces, a breaker sitting `open` more than `sentinel.breakerStuckMs` (30min) past its own cooloff with no cycle progress since is treated as a wedge. Env overrides: `COSMO23_SENTINEL_{CHECK_INTERVAL_MS,WEDGE_THRESHOLD_MS,LAUNCH_GRACE_MS,MAX_ATTEMPTS,ATTEMPT_TTL_MS,BREAKER_STUCK_MS}`; config block `sentinel:` in `~/.cosmo2.3/config.json`.

**Effect standalone:** backward-compatible; existing consumers of `running`/`health.lifecycle` unaffected. **Tests:** `tests/cosmo23/run-sentinel.test.cjs` (ladder, bounds, escalation, grace, persistence, cleanup, watchdog hand-off/phase awareness/stuck-breaker) + existing `server/lib/status-contract.test.js` still green.

**User-intent follow-up (2026-07-22, fix-first review of the initial landing):** a user `POST /api/stop` is FINAL — it can never be undone by the sentinel. `/api/stop` now notifies the sentinel with `userInitiated: true` in BOTH branches (active run and `not_running` — the latter covers a sentinel remediation that already cleared the context, and a failed relaunch's pending retry). The user-stop path force-clears the ladder, pending relaunch, and in-memory wedged/escalated state even while a remediation is in flight; the in-flight remediation re-checks the user-stop epoch between its engine stop and relaunch and aborts (`aborted_user_stop`) instead of resurrecting the run. On user stop, `<runDir>/.sentinel.json` is archived to `.sentinel.json.last` (evidence is never silently deleted), which also clears a sticky `wedged: true` on escalated dead runs (no cosmo-exit ever fires for them). Fresh-launch amnesty: a NEW launch whose `startedAt` postdates the last recorded attempt/escalation loads with a reset ladder — it does not inherit the previous run's wedged flag, and a real wedge gets a fresh bounded ladder (the sentinel's own relaunches keep in-memory state, so the ladder stays bounded across remediations).
...
## Patch 73 — per-run config.yaml carries no provider credentials (2026-08-11)

**Why:** `launcher/config-generator.js` interpolated live `apiKey:` values for minimax and ollama-cloud into every generated `runs/<name>/config.yaml`. Run dirs are feeder-watched trees on the Home23 side (agent `workspace/research-runs/` entries resolve into `cosmo23/runs/`), and Home23's feeder ingests unknown non-binary extensions as text — live provider keys were one flush away from being compiled into an agent's brain graph. 31 historical run configs held real keys; all were scrubbed to `""` the same day, and a fragment sweep across every jerry brain sidecar (including `state.json.gz` and the memory base/delta) confirmed nothing had been ingested. Feeder exclusions for `**/config.yaml` were added on the Home23 side in the same pass.

**What changed (config surface):** the generated `providers.minimax` and `providers.ollama-cloud` blocks no longer contain `apiKey:` lines. The engine's existing env fallbacks in `engine/src/core/unified-client.js` (`minimaxConfig.apiKey || process.env.MINIMAX_API_KEY` at :97; `cloudConfig.apiKey || process.env.OLLAMA_CLOUD_API_KEY` at :137) carry authentication — exactly the contract the openai provider has always used (enabled-flag only, key from env). Under Home23, both vars are PM2-injected into the cosmo23 server and inherited by spawned engine subprocesses; standalone cosmo23 users must export them, same as `OPENAI_API_KEY`.

**Effect standalone:** an install that relied on the interpolated disk key with no env var set now fails loudly at provider init ("no API key found") instead of silently authenticating from a credential on disk — accepted, same doctrine as the `31d6afa2` broker-shim deletion (fail loud over boot-time copies). Continuations regenerate `config.yaml` through this generator, so historical configs heal on their next continue. **Tests:** none added — a string-emission change exercised by every launch/continue.

## Patch 74 — brain-provider registry rebuilds on credential rotation (2026-08-11)

**Why:** `lib/brain-provider-client-registry.js` bound provider credentials at registry construction, and the registry is built once at dashboard boot (plus on Settings-driven authority refresh). Since a token rotation now restarts nothing (`rotationRestartTargets` returns `[]`), a rotated `sk-ant-oat*` never reached the dashboard's brain-operation clients until an unrelated restart — the one consumer of `secrets.yaml` still frozen-by-value after the 2026-08-10/11 read-at-use work.
...
## Patch 75 — guided Launch is one disk-backed research drill (2026-08-18)

**Why:** live model trials showed the legacy guided lifecycle could fetch and think for many cycles without landing phase writeups, expose empty Sources/Brain panels while workers were active, stop on engine-cycle limits unrelated to the research budget, and report Idle while the run still had drill work. A first drill cut established the right direction but accepted any markdown in `outputs/` for every phase and still ran the legacy cognitive watchdog beside the product drill.

**What changed:** fresh guided Launch persists a short tool-loop contract (including explicit `PHASE N` lanes from the launch context), then gives one `DrillLoop` sole ownership of the engine lifecycle. It chains goal → parallel phase workers → merged next goal until the launch cycle/time budget settles. Legacy specialist plans and their planner remain available for old continuations. Worker tools journal every successful fetch to `outputs/sources.jsonl`; goals, phase transitions, thoughts, harvests, findings, offshoots, and writeups append to `outputs/stream.jsonl` before entering live memory. `stream-brain.jsonl` acknowledges successful Brain writes, unacknowledged rows replay on restart, and each settled descent checkpoints the Brain. Phase closure now requires a substantive markdown write receipt bound to that exact goal/phase and still verifiable under `outputs/`; another phase's file, tape, and temporary dumps cannot close it. A fresh worker gets bounded research turns, but a worker inheriting phase tape or a write-now operator note receives required `write_file` on its first model turn; after the file lands the harness requires `remember`, then `finish`, so “writing now” cannot loop as hidden thought. Operator notes remain durable and reach every later worker.

The standalone Watch surface remains the desk and now reads the existing drill state, tapes, and files through read-only APIs. It shows lifecycle, goal, phases, workers, remaining budgets, bounded tape/source counts (explicitly marked as lower bounds when history exceeds the read window), writeups, files, previews, and durable steering notes. Brains, Query, Map, Intelligence, Interactive, and Ingest remain intact; Interactive is still chat-only.

**Lifecycle and launch safety:** product drills do not enter legacy cognitive cycles, sleep/governance, engine max-cycle checks, or the cycle watchdog. Worker/coordinator model calls have their own bounded timeout and the drill owns its budgets. `DrillLoop.stop()` persists `stopped`, clears active worker claims, and returns active phases to pending before orchestrator shutdown proceeds. Unexpected engine exit appends interruption evidence and persists `interrupted`; a missed exit callback is repaired by the next offline status read, with derived interrupted API truth if disk persistence itself fails. Natural `done` and deliberate `parked` states are preserved. Child readiness is port-backed with a 10-second bound. An atomically-created `drill/runner.claim`, current `runner.json`, and append-only `runner.jsonl` record process ownership without erasing stale evidence; a live claim/PID or occupied engine WebSocket port fails closed instead of dual-starting a runner. Dashboard/MCP startup refuses occupied ports instead of killing unknown owners. Anthropic engine auth reads the token selected in Cosmo Setup before considering a possibly stale inherited PM2 token; direct SQLite decryption remains available when Prisma generation is unavailable.
