...
- Canonical resident state now projects open resident tasks and handoffs. `state.openTasks`, `state.attention.openTasks`, and `open_task` obligations let chat/dashboard hydrate active action debt directly from `state.json`, not only from the task API.
