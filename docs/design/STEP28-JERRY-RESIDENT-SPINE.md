...
- Raw body telemetry observations such as pressure and health readings follow the same scarcity rule. They remain evidence for verifiers and pursuits, but cannot spend active/watch attention as standalone body-sensor rows unless they declare a changed future or stop condition.
- Raw agenda status observations follow the same scarcity rule. Agenda can still be an action surface and evidence source, but stale/status rows do not become resident pursuits unless they bind to a changed future or stop condition.
...
- Source-truth decay now has teeth: expired current claims append a `stale` truth row, `truth_claim_decayed` receipt, and `stale_claim_demoted` consequence instead of merely disappearing from current-state projection.
...
- Canonical resident state projects active obligations with an explicit audience. Authority requests and operator questions are owed by jtr; open tasks and truth contradictions are owed by the resident; blocked pursuits are operator-facing only when their blocker identifies jtr, the operator, or an authority decision. The success-test brief and dashboard keep those sets separate as "What I need from jtr" and "What I owe."
- Canonical resident state now carries compact active pursuits, current pursuit, watchlist entries with evidence, and recent consequences. Chat and dashboard can hydrate the live contract from `state.json` without reconstructing the resident spine from scattered ledgers.
- Chat pursuit updates now mutate the resident living thread, not just a status log. `agency_update_pursuit` can update summary, current theory, next move, changed future, stop condition, what-would-change-my-mind, and evidence through the bridge; the kernel persists those fields on the pursuit itself with transition receipts.
...
- Canonical resident state now projects open resident tasks and handoffs. `state.openTasks`, `state.attention.openTasks`, and `open_task` obligations let chat/dashboard hydrate active action debt directly from `state.json`, not only from the task API. A task that did not declare authority remains `unknown`, never an inferred L2 escalation. Coordination task projections are omitted from obligations only when their canonical Work carries explicit terminal evidence; absent or unfamiliar Work state stays open.
...
- COSMO research outputs are no longer only chat answers or markdown artifacts waiting for feeder ingestion. `research_query_brain`, `research_search_all_brains`, `research_get_brain_summary`, `research_get_brain_graph`, `research_compile_brain`, and `research_compile_section` now emit `cosmo.research` world-stream packets with research evidence, changed-future text, and resident intake outcomes.
- From The Inside publish verification now requires a cited recent agency consequence from resident state. A public issue cannot pass evidence verification on byte/render checks alone; it must show the newsletter/public artifact changed because Jerry lived through a concrete system consequence.
- From The Inside public publishing now has pre-write veto power. `publish-public-issue.py` refuses both dry-run acceptance and `--apply` writes unless the issue source and rendered HTML cite a recent resident agency consequence; post-publish verification remains the second gate.
...
- Structured report packets can emit selected `claims` or `beliefUpdates` directly into the source-truth hierarchy. These create `world_stream_child_claim_recorded` receipts and `belief_updated` consequences, so a report can update Jerry's standing beliefs without pretending every claim is a contradiction.
- Structured report packets can also emit `memoryCandidates`, giving reports a durable non-claim memory route. These items write `memory-candidates.jsonl`, `memory_candidate_created` consequences, and `world_stream_child_memory_candidate_created` receipts without pretending provisional knowledge is a verified truth claim.
- Structured report packets can emit `operatorQuestions` for the "ask fewer, better questions" contract. These become `jtr_question_raised` receipts/consequences plus `world_stream_child_question_raised` receipts, so report uncertainty is bounded and inspectable instead of becoming conversational anxiety.
- Structured report packets can emit `taskItems` or `tasks` for bounded action and handoff outcomes. These items route through `AgencyKernel.recordTask()`, write `tasks.jsonl`, and add `world_stream_child_task_created` receipts so reports can produce worker handoffs without pretending the handoff itself is the completed consequence.
...
- `AgencyKernel.brief()` and `GET /api/agency/brief` answer the Step28 success-test question from live resident state: what Jerry is following, what changed, what he is doing next, what needs jtr, and what the resident owes. Chat exposes this through `agency_brief`, and the dashboard renders the same split brief.
- Pending unsurfaced operator obligations are a small always-on per-turn context block, separate from the keyword-gated full Agency surface. It emits nothing when state is empty or unreadable, sorts oldest first, and caps itself at five items plus an overflow count.
- After the resident actually voices an obligation to jtr, `agency_brief` may POST its explicit obligation ID. The kernel appends `obligation_surfaced_to_jtr`; context display alone never writes that receipt, and surfacing does not resolve or close the underlying question, authority request, or pursuit.
