...
- COSMO research outputs are no longer only chat answers or markdown artifacts waiting for feeder ingestion. `research_query_brain`, `research_search_all_brains`, `research_get_brain_summary`, `research_compile_brain`, and `research_compile_section` now emit `cosmo.research` world-stream packets with research evidence, changed-future text, and resident intake outcomes.
