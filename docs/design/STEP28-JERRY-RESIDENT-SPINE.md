...
- COSMO research compiles are no longer only markdown artifacts waiting for feeder ingestion. `research_compile_brain` and `research_compile_section` now emit `cosmo.research` world-stream packets with research evidence, changed-future text, and resident intake outcomes.
