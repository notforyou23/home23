...
- Tasks and routed handoffs are now first-class action records. `tasks.jsonl`, `AgencyKernel.recordTask()`, `GET/POST /api/agency/tasks`, dashboard proxying, chat tool `agency_create_task`, and live low-risk `task_created` deltas create open resident task/action receipts without pretending the work is already complete.
