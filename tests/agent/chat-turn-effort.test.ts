...
test('chat-turn rejects none for GPT-6 Astra before starting the turn', async () => {
  const captured: { effort?: unknown } = {};
  const app = express();
  app.use(express.json());
  app.post('/api/chat/turn', createTurnStartHandler({
    ...
    history: makeFakeHistory() as any,
    modelAliases: {
      astra: { provider: 'openai-codex', model: 'gpt-6-astra' },
    },
  } as any));

  const res = await postJson(app, {
    chatId: 'c1', message: 'hello', model: 'astra', effort: 'none',
  });
  assert.equal(res.status, 400);
  assert.equal(res.body.code, 'reasoning_effort_unsupported');
  assert.equal(captured.effort, undefined);
});
