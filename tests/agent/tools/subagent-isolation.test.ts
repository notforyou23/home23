...
import {
  ...
  WORKING_THREAD_RESULT_MAX_CHARS,
  spawnAgentTool,
...
test('canonical Connected Agents explicit detached mode captures exact durable destination', async () => {
  ...
  assert.deepEqual(creates, [{
    kind: 'subagent', taskBrief: 'background this', originChatId: 'coordination:cnv_forrest:work_456',
...
test('specialists nested inside a canonical Working Thread retain the wrk root and cannot deliver interim chat results', async () => {
  const { ctx, captured } = makeCtx('coordination:chn_jerry:wrk_root');
  const root = {
    kind: 'coordination' as const,
    parentWorkId: 'wrk_root',
    channelId: 'chn_jerry',
    conversationId: 'cnv_jerry',
    originMessageId: 'msg_owner',
    attemptId: 'att_root',
    leaseId: 'lse_root',
    fencingToken: 4,
    targetPrincipalId: 'bot_jerry',
    residentBinding: 'jerry',
    residentInstanceId: 'resident-jerry-1',
    authorityReference: 'resident:jerry',
  };
  ctx.parentWorkId = root.parentWorkId;
  ctx.coordinationWorkDestination = root;
  const creates: any[] = [];
  const inlineCompletions: string[] = [];
  let next = 0;
  (ctx as ToolContext & { workRegistry: NonNullable<ToolContext['workRegistry']> }).workRegistry = {
    create: (input: any) => {
      creates.push(input);
      next += 1;
      return { workId: `aw_nested_${next}`, originChatId: ctx.chatId };
    },
    completeInline: (workId: string) => { inlineCompletions.push(workId); return {}; },
  } as NonNullable<ToolContext['workRegistry']>;
  let terminalCalls = 0;
  ctx.onWorkTerminal = () => { terminalCalls += 1; };

  const first = await spawnAgentTool.execute({ task: 'first internal hand', mode: 'detached' }, ctx);
  assert.equal(first.content, 'sub result');
  assert.equal(captured.ctx?.parentWorkId, root.parentWorkId);
  assert.deepEqual(captured.ctx?.coordinationWorkDestination, root);

  const second = await spawnAgentTool.execute(
    { task: 'second nested hand', mode: 'detached' },
    captured.ctx!,
  );
  assert.equal(second.content, 'sub result');
  assert.equal(creates.length, 2);
  for (const created of creates) {
    assert.equal(created.parentWorkId, root.parentWorkId);
    assert.equal(created.deliveryMode, 'inline');
    assert.equal(created.coordinationDestination, undefined,
      'hidden hands never own the root result destination');
  }
  assert.deepEqual(inlineCompletions, ['aw_nested_1', 'aw_nested_2']);
  assert.equal(terminalCalls, 0, 'only the accountable root Work may post a canonical result');
  assert.equal(captured.appends.length, 0, 'nested completion never appends an assistant transcript row');
});
...
test('canonical exact specialist preserves full evidence through owner synthesis with a bounded Work budget', async () => {
  const destination = {
    kind: 'coordination' as const, parentWorkId: 'wrk_root', channelId: 'chn_jerry',
    conversationId: 'cnv_jerry', originMessageId: 'msg_owner', attemptId: 'att_root',
    leaseId: 'lse_root', fencingToken: 1, targetPrincipalId: 'bot_jerry',
    residentBinding: 'jerry', residentInstanceId: 'resident-jerry-1', authorityReference: 'resident:jerry',
  };
  for (const length of [3058, WORKING_THREAD_RESULT_MAX_CHARS, WORKING_THREAD_RESULT_MAX_CHARS + 1]) {
    const { ctx } = makeCtx('coordination:chn_jerry:wrk_root');
    const exact = 'Evidence '.repeat(Math.ceil(length / 9)).slice(0, length);
    const events: Array<Record<string, unknown>> = [];
    let calls = 0;
    let synthesisEvidence = '';
    ctx.runAgentLoop = async (_system, message, _tools, _context, options) => {
      calls++;
      assert.equal((options?.registry as ReturnType<typeof createSeededToolRegistry>).getOpenAITools().length, 0);
      if (calls === 1) {
        assert.match(message, /no longer than 24000 characters/);
        return { text: exact, model: 'test', toolCallCount: 0, durationMs: 1 };
      }
      synthesisEvidence = message.split('Completed tool evidence:\n')[1]!;
      return { text: 'The supplied reliability review is complete.', model: 'test', toolCallCount: 0, durationMs: 1 };
    };
    const execution = executePlannedTool({
      registry: createSeededToolRegistry([spawnAgentTool]), name: 'spawn_agent',
      toolCallId: 'call_work_review', canonicalArgs: { task: 'Review supplied facts.', mode: 'joined', isolated: true, tool_grants: [] },
      context: ctx, destination, attemptChatId: ctx.chatId, abortController: new AbortController(),
      onEvent: event => events.push(event as unknown as Record<string, unknown>),
    });
    if (length > WORKING_THREAD_RESULT_MAX_CHARS) {
      await assert.rejects(execution, /exceeded 24000 characters/);
      assert.equal(calls, 1);
      assert.equal(events.find(event => event.type === 'tool_result')?.success, false);
    } else {
      const result = await execution;
      assert.equal(result.text, 'The supplied reliability review is complete.');
      assert.equal(calls, 2);
      assert.equal(synthesisEvidence, exact);
      assert.equal(events.find(event => event.type === 'tool_result')?.exactResult, exact);
      assert.equal(events.find(event => event.type === 'subagent_result')?.result, exact);
    }
  }
});


test('spawn_agent rejects absent or empty tasks without dispatching or consuming a slot', async () => {
  for (const task of [undefined, null, '', '   ', 123]) {
    const { ctx, captured } = makeCtx('parent-chat');
    const result = await spawnAgentTool.execute({ task }, ctx);
    assert.equal(result.is_error, true);
    assert.match(result.content, /non-empty task/);
    assert.equal(captured.loopCalls, 0);
    assert.equal(ctx.subAgentTracker.active, 0);
  }
});

test('spawn_agent reports concurrency refusal as an error without dispatch', async () => {
  const { ctx, captured } = makeCtx('parent-chat');
  ctx.subAgentTracker.active = ctx.subAgentTracker.maxConcurrent;
  const result = await spawnAgentTool.execute({ task: 'another task' }, ctx);
  assert.equal(result.is_error, true);
  assert.equal(captured.loopCalls, 0);
  assert.equal(ctx.subAgentTracker.active, ctx.subAgentTracker.maxConcurrent);
});
