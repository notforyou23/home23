...
for (const slug of ['jerry', 'forrest'] as const) test(`signed channel operations derive ${slug} identity and retry the same canonical mutation`, async (t) => {
  ...
  const consume = createChannelOperationConsumer({ channels,
    authorize: (credential, received) => { assert.equal(credential.residentSlug, slug); assert.equal(received, origin); if (!valid) throw new Error('stale fence'); },
    context: () => residentContext(fixture.bots[slug], slug, 990),
    listBots: () => fixture.directory.listVisibleBots(),
    botOperation: async () => { throw new Error('unused'); },
  });
  const credential = { residentSlug: slug, instanceId: 'client', keyVersion: 1 };
  ...
  assert.ok(first.channel.members.some((member: any) => member.principalId === fixture.bots[slug].id));
  assert.equal(first.channel.responderPolicy.coordinatorBotId, fixture.bots[slug].id);
  valid = false;
