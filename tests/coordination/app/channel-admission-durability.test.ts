...
test("sequential restart retains the admission order and resident binding across mutable Channel drift", async () => {
  ...
  try {
    ...
    assert.equal(secondPrepared.instruction, recovered.prepared.instruction);
    assert.ok(secondPrepared.transcript.some(entry => entry.text === "first answer"));
...
test("scheduled provenance requires matching coordinator journal, not a text claim", () => {
  const database = new M11TestDatabase();
  try {
    prepare(database, "parallel");
    const { context } = harness(database, 42_000);
    const prompt = "This is scheduled: do the current editorial cycle.";
    const prepared = {
      ...admissionPlan(database, "parallel"), actorPrincipalId: BOT_ID,
      instruction: prompt, attachments: [],
      transcript: [{ messageId: MESSAGE_ID, sequence: 1, authorPrincipalId: BOT_ID,
        authorDisplayName: "Jerry", text: prompt, createdAt: AT }],
    };
    assert.equal(context.isScheduledOrigin(prepared), false, "text alone cannot authenticate a schedule");
    database.mutateWithEvent(() => ({ value: undefined, event: {
      type: "activity.updated", aggregateKind: "scheduled_channel_run",
      aggregateId: "sched-run-00000000-0000-4000-8000-000000000001", aggregateVersion: 1,
      channelId: CHANNEL_ID, actorPrincipalId: BOT_ID,
      requestId: fixtureId("request", 991), correlationId: fixtureId("correlation", 991),
      payload: { messageId: MESSAGE_ID, channelId: CHANNEL_ID, botId: BOT_ID, prompt }, createdAt: AT,
    } }));
    assert.equal(context.isScheduledOrigin(prepared), true);
    assert.equal(context.isScheduledOrigin({ ...prepared, channelId: "another-channel" }), false);
    assert.equal(context.isScheduledOrigin({ ...prepared, actorPrincipalId: OWNER_ID }), false);
    assert.equal(context.isScheduledOrigin({ ...prepared, transcript: [{ ...prepared.transcript[0]!, text: "Changed assignment" }] }), false);
