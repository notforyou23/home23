...
test('historical direct Bot outcome waits while group and current outcomes remain routable', () => {
  ...
  try {
    sqlite.exec(`CREATE TABLE channels (id TEXT PRIMARY KEY, kind TEXT);
      ...
      INSERT INTO bots VALUES ('bot-1','bot-helper','active','current-conversation'),
        ('bot-archived','bot-archived-helper','archived','current-conversation'),
        ('resident','jerry','active','current-conversation');`);
