...
test("authenticated direct Message follows one durable M08/M11/M13 correlation chain", async (t) => {
  ...
  await t.test("old accepted retry survives transcript eviction and unavailable current catalog", async () => {
    const originalManifest = submitted.work.contextManifestId;
    for (let index = 0; index < 105; index += 1) {
      await messages.sendMessage({
        context: owner, channelId: CHANNEL_ID, messageId: fixtureId("message", 50_000 + index),
        authorPrincipalId: OWNER_ID, idempotencyKey: `later-message-for-replay-${index}`,
        kind: "text", text: `Later independent message ${index}`, mentions: [], attachmentIds: [],
        clientMessageId: `later-${index}`, replyToMessageId: null, tombstonesMessageId: null,
        provenance: { roundId: null, workId: null },
      });
    }
    const priorCatalog = agent.modelCatalog;
    agent.modelCatalog = async () => { throw new Error("current resident is offline"); };
    try {
      const request = {
        context: owner, channelId: CHANNEL_ID, idempotencyKey: "m14-direct-message-0001",
        body: { messageId: fixtureId("message", 900), clientMessageId: "client-m14-1",
          text: "Jerry, answer canonically.", attachmentIds: [], mentions: [], replyToMessageId: null,
          modelAlias: "sol", reasoningEffort: "xhigh" as const },
      };
      const attachmentsBefore = residentAttachments;
      const retry = await service.submitMessage(request);
      assert.equal(retry.replayed, true);
      assert.equal(retry.work.id, submitted.work.id);
      assert.equal(retry.work.contextManifestId, originalManifest);
      assert.equal((await retry.response).id, response.id);
      assert.equal(residentAttachments, attachmentsBefore);
      await assert.rejects(service.submitMessage({ ...request, body: { ...request.body, text: "changed body" } }),
        (error: unknown) => (error as { code?: string }).code === "idempotency_conflict");
      await assert.rejects(service.submitMessage({ ...request, body: { ...request.body, modelAlias: "changed-model" } }),
        (error: unknown) => (error as { code?: string }).code === "idempotency_conflict");
    } finally {
      agent.modelCatalog = priorCatalog;
    }
  });

  await t.test("accepted active and cancelling retries acknowledge without current target readiness", async () => {
    for (const state of ["queued", "leased", "running", "cancelling"] as const) {
      ...
      const recover = context.recover.bind(context);
      context.recover = async (prior) => { contextRecoveries += 1; return recover(prior); };
      const replayService = createDirectMessageSubmissionService({
        messages, context, work: { ...work, get: (id) => {
          const existing = work.get(id);
          return existing && id === submitted.work.id ? { ...existing, state } : existing;
        } }, leases, communications, authority, beginWork,
        resolveExecutionTarget: async () => { targetLookups += 1; await targetGate; return undefined; },
        resolveResident: () => undefined,
        recoveryIdentity: () => ({ requestId: fixtureId("request", 904), correlationId: fixtureId("correlation", 904) }),
      });
      const attachmentsBefore = residentAttachments;
      const retry = await replayService.submitMessage({
        context: owner, channelId: CHANNEL_ID, idempotencyKey: "m14-direct-message-0001",
        body: { messageId: fixtureId("message", 900), clientMessageId: "client-m14-1",
          text: "Jerry, answer canonically.", attachmentIds: [], mentions: [], replyToMessageId: null,
          modelAlias: "sol", reasoningEffort: "xhigh" },
      ...
      assert.equal(retry.work.id, submitted.work.id);
      assert.equal(retry.work.state, state);
      assert.equal(retry.replayed, true);
      releaseTarget();
      await assert.rejects(retry.response, state === "cancelling" ? /cancelling/ : /target is not enabled/);
      assert.equal(contextRecoveries, state === "cancelling" ? 0 : 1);
      assert.equal(targetLookups, state === "cancelling" ? 0 : 1);
      assert.equal(residentAttachments, attachmentsBefore);
      assert.equal(activeBackgroundWork, 0);
    }
  });
