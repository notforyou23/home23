...
test("canonical push durably retries and duplicate recovery repairs a missing delivery", async (t) => {
  ...
  const pusher = new ApnsPusher({
    send: async (_token: string, payload: { aps: { badge: number } }) => {
      badges.push(payload.aps.badge);
      attempts += 1;
    ...
    connectedAgentsBadgeCount: () => 3 - attempts,
  });
  const notifications = new ConnectedAgentsNotificationService(
    registry,
    pusher,
  ...
  assert.deepEqual(badges, [3, 2, 1], "each retry uses current unread rather than stale payload state");
