...
test('authenticated Jerry manages member channels with his own durable authorship', async (t) => {
  ...
  const input = { context: residentContext(fixture.bots.jerry, 'jerry', 901),
    title: 'Editorial', purpose: 'Persistent editorial work', memberBotIds: [fixture.bots.jerry.id],
    pinned: false, responderPolicy: { mode: 'mentions_only' as const, coordinatorBotId: null,
      responseOrder: 'sequential' as const, maxBotTurns: 2 }, idempotencyKey: channelKey(901) };
  const created = await channels.createGroupChannel(input);
  const replay = await channels.createGroupChannel(input);
  assert.equal(replay.channel.id, created.channel.id);
  const updated = await channels.updateChannel({ ...input,
    context: residentContext(fixture.bots.jerry, 'jerry', 902), idempotencyKey: channelKey(902),
    channelId: created.channel.id, expectedVersion: created.channel.version,
    memberBotIds: [fixture.bots.jerry.id, fixture.bots.forrest.id], lifecycle: 'active' });
  assert.equal(updated.channel.members.length, 3);
