...
test("concurrent sends allocate unique gap-free per-Channel sequences", async (t) => {
  ...
  const committed = await Promise.all(Array.from({ length: 64 }, (_, index) =>
  ...
  assert.equal(new Set(committed.map((result) => result.message.id)).size, 64);
  ...
  const latestPage = await messages.listMessages({
    context: ownerContext(498),
    channelId: direct.channel.id,
    limit: 50,
  });
  assert.equal(latestPage.messages.length, 50);
  assert.equal(latestPage.messages.some((message) => message.id === committed[0]!.message.id), false);
  ...
  await assert.rejects(
    messages.getMessage({
      context: residentContext(fixture.bots.forrest, "forrest", 501),
      messageId: committed[0]!.message.id,
    }),
