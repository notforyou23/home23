...
test('Jerry creates and archives helpers with verified resident binding and his own audit identity', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'home23-executive-bots-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const databasePath = join(root, 'coordination.sqlite3');
  await bootstrapJerry({ databasePath, apply: true,
    authority: { approved: true, kind: 'm14-bootstrap', operator: 'user_owner', resident: 'jerry',
      legacyWriterAuthoritative: true, coordinationFlagsAllFalse: true },
    serverInstanceId: 'home23-jerry-harness', keyVersion: 1, now: () => new Date(NOW) });
  appendAuthorityHistory(databasePath);
  const database = openCoordinationDatabase({ path: databasePath, now: () => new Date(NOW) });
  t.after(() => database.close());
  const principalId = database.readOne<{ id: string }>("SELECT id FROM bots WHERE resident_binding = 'jerry'")!.id;
  const executiveActor = { principalId, kind: 'bot' as const, displayName: 'Jerry',
    requestId: generateCoordinationId('request'), correlationId: generateCoordinationId('correlation'),
    residentCredential: { residentBinding: 'jerry', instanceId: 'home23-jerry-harness', keyVersion: 1 } };
  const service = createBotLifecycleService({ authority: { enabled: () => true,
    currentEpoch: async () => canonicalEpoch('bot_lifecycle'), decide: request => classifyPolicy(request, new Date(NOW)) },
    mailboxBinder: new SqlitePersistentMailboxBinder({ database, now: () => new Date(NOW) }),
    receipts: new SqliteBotLifecycleReceiptStore(database), canonicalWriter: COORDINATION_BOT_LIFECYCLE_WRITER,
    now: () => new Date(NOW) });
  const requestId = 'executive-create-helper-01';
  const target = derivePersistentBotBinding({ requestId, displayName: 'Editor' });
  const createPolicy = policy('bot_lifecycle.create', target);
  const request = { requestId, correlationId: executiveActor.correlationId, actorPrincipalId: principalId,
    executiveActor, displayName: 'Editor', purpose: 'Editorial help', expectedAuthorityEpoch: 3,
    policy: { ...createPolicy, action: { ...createPolicy.action, actorPrincipalId: principalId } } };
  const created = await service.create(request);
  assert.equal(created.outcome, 'succeeded');
  const event = database.readOne<{ actor: string }>("SELECT actor_principal_id AS actor FROM events WHERE type = 'bot.created' AND aggregate_id = ?", created.botId!);
  assert.equal(event?.actor, principalId);
  const controlPolicy = policy('bot_lifecycle.archive', created.botId!);
  const archived = await service.control({ requestId: 'executive-archive-helper-01', correlationId: executiveActor.correlationId,
    actorPrincipalId: principalId, executiveActor, botId: created.botId!, operation: 'archive', expectedAuthorityEpoch: 3,
    policy: { ...controlPolicy, action: { ...controlPolicy.action, actorPrincipalId: principalId } } });
  assert.equal(archived.outcome, 'succeeded');
  const badKey = 'executive-create-stale-01';
  const staleTarget = derivePersistentBotBinding({ requestId: badKey, displayName: 'Editor' });
  await assert.rejects(service.create({ ...request, requestId: badKey,
    executiveActor: { ...executiveActor, residentCredential: { ...executiveActor.residentCredential, keyVersion: 2 } },
    policy: { ...request.policy, action: { ...request.policy.action, target: staleTarget } } }),
    (error: unknown) => error instanceof BotLifecycleError && error.code === 'operation_failed');
});
