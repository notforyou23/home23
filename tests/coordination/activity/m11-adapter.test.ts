...
test("a durable resident_work_thread retains resident Activity authority", () => {
  const adapted = adaptTrustedM11ActivityFact(fact(1, {
    workKind: "resident_work_thread",
    executionAuthoritySystem: "resident_turn",
  }));
  assert.equal(adapted?.observation.authoritySystem, "resident_turn");
  assert.equal(adapted?.binding.workId, WORK);
});
