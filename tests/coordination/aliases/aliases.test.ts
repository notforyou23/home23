...
test("the same exact alias and target is idempotently already bound", () => {
  ...
  const plan = planAliasBinding([existing], {
    aliasId: "alias_0198d95f-6c00-7000-8000-000000000095",
    namespace: "legacy-session",
    legacyId: "session-42",
    targetType: "message",
    targetId: MESSAGE_ID,
...
test("inactive aliases stay reserved and stored collisions fail closed", () => {
  ...
  const rebinding = planAliasBinding([inactive], {
    ...
    targetType: "message",
    targetId: MESSAGE_ID,
    provenance: inactive.provenance,
  });
  assert.equal(rebinding.decision, "denied");
  assert.equal(rebinding.reason, "inactive_alias_reserved");

  const collided = resolveAlias(
    [binding(), binding({ aliasId: "alias_0198d95f-6c00-7000-8000-000000000095", targetId: OTHER_MESSAGE_ID })],
    "legacy-session",
    "session-42",
  );
  ...
  const activeAndInactive = resolveAlias(
    [binding(), binding({ aliasId: "alias_0198d95f-6c00-7000-8000-000000000096", active: false })],
    "legacy-session",
    "session-42",
  );
  ...
  const inactiveCollision = resolveAlias(
    [inactive, binding({ aliasId: "alias_0198d95f-6c00-7000-8000-000000000096", active: false })],
    "legacy-session",
    "session-42",
  );
...
test("malformed stored aliases fail closed before resolution", () => {
  ...
  const malformedProvenance = binding({
    provenance: {
      sourceId: "not-a-source-id",
      importKeyDigest: "not-a-digest",
    },
  });
  assert.deepEqual(
    resolveAlias([malformedProvenance], "legacy-session", "session-42"),
    { decision: "denied", reason: "invalid_stored_alias" },
  );

  const replanning = planAliasBinding([malformedProvenance], {
    aliasId: ALIAS_ID,
    namespace: "legacy-session",
    legacyId: "session-42",
    targetType: "message",
