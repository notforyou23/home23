...
test("loopback product API traverses canonical temp-db Bots, Channels, Messages, Inbox, and Unread", async (t) => {
  ...
  const topicBody = {kind: "group", title: "Guitar practice", purpose: "A durable subject", memberBotIds: [fixture.bots.jerry.id], pinned: false, responderPolicy: {mode: "mention_or_coordinator", coordinatorBotId: fixture.bots.jerry.id, responseOrder: "sequential", maxBotTurns: 2}};
  const topicResponse = await fetch(`${address.origin}/api/v1/channels`, {method: "POST", headers: headers("topic-create-000001"), body: JSON.stringify(topicBody)});
  ...
  assert((await inbox.json() as any).conversations.some((row: any) => row.channelId === channel.id));
