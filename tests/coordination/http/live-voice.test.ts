...
test("canonical voice routes authenticate, validate bodies and preserve exact opaque handles", async t => {
  ...
  assert.equal((await post(`${base}/sessions`, { sdp: "offer", voice: "invented" })).status, 400);
  assert.equal((await post(`${base}/sessions`, { sdp: "offer", speakingPace: 2 })).status, 400);
  assert.equal((await post(`${base}/sessions`, { sdp: "offer", speakingPace: "rushed" })).status, 400);
  assert.equal((await post(`${base}/sessions`, { sdp: "x".repeat(65_537) })).status, 400);
  const started = await post(`${base}/sessions`, { sdp: "offer", modelAlias: "chosen", reasoningEffort: "high", voice: "cedar", speakingPace: "slower" });
  ...
  assert.equal(calls.at(-1)?.input.voice, "cedar");
  assert.equal(calls.at(-1)?.input.speakingPace, "slower");
