...
test('engine MCP HTTP server binds only to loopback and exposes health', async () => {
  ...
  try {
    ...
    resolveReadiness({ ok: true, sourceHealth: 'healthy', revision: 7 });
...
test('engine MCP health is unavailable when the canonical source check fails', async () => {
  const server = startMcpHttpServer({
    port: 0,
    host: '127.0.0.1',
    log: false,
...
test('engine MCP memory tools delegate to the bounded canonical-source adapter', async () => {
  const calls = [];
  const memoryTools = {
