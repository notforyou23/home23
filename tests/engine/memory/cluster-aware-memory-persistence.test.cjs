...
const { MemoryDiffMerger } = require('../../../engine/src/cluster/memory-merger.js');
const { MemorySummarizer: HomeMemorySummarizer } = require('../../../engine/src/memory/summarizer.js');
...
test('Home23 summarizer consolidation and garbage collection publish through mutation APIs', async () => {
  const { base } = createMemory();
  ...
  base.markPersistenceCleanIfGeneration(base.persistenceGeneration);
  const generation = base.persistenceGeneration;
  const summarizer = new HomeMemorySummarizer({}, {
