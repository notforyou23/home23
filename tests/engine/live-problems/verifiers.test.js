...
test('create_file_tool_probe serializes concurrent probes targeting one fixed path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'home23-create-file-probe-race-test-'));
  const modulePath = path.join(dir, 'tools.cjs');
  fs.writeFileSync(modulePath, `
    const fs = require('fs').promises;
    const path = require('path');
  ...
  try {
    const results = await Promise.all(Array.from({ length: 100 }, (_, index) => runVerifier({
      type: 'create_file_tool_probe',
      args: {
        modulePath,
    ...
    assert.equal(results.filter(result => result.ok).length, results.length,
