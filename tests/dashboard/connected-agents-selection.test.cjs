...
const wire = (channelId = 'chn_0198d95f-6c00-7000-8000-000000000001') => ({
  ...
  models: [
    ...
    {
      alias: 'astra', provider: 'openai-codex', model: 'gpt-6-astra', reasoningEffort: null,
...
test('execution catalog validation binds exact aliases and efforts to one Channel', () => {
  ...
  assert.deepEqual(options.models.map((model) => model.alias), ['sol', 'terra', 'astra']);
...
test('model-specific effort capabilities preserve Astra and clear only its unsupported effort', () => {
  const options = Selection.normalizeOptions(wire());
  assert.deepEqual(
    Selection.reasoningEffortsForModel(options, 'astra'),
    ['low', 'medium', 'high', 'xhigh', 'max'],
  );
  assert.deepEqual(Selection.normalizePreference({
    modelAlias: 'astra', reasoningEffort: 'none',
  }, options), { modelAlias: 'astra', reasoningEffort: null });
  assert.deepEqual(Selection.normalizePreference({
    modelAlias: 'sol', reasoningEffort: 'none',
  }, options), { modelAlias: 'sol', reasoningEffort: 'none' });
});
