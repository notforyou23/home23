...
test('Agency inspector exposes cron retirement proposals as a filtered proof-chain surface', () => {
  ...
  const agencyReceiptRow = js.match(/function renderAgencyReceiptRow\(r\)[\s\S]*?(?=\nfunction renderAgencyInboxRow)/)?.[0] || '';
  const agencyInboxRow = js.match(/function renderAgencyInboxRow\(c\)[\s\S]*?(?=\nfunction renderAgencyCandidateTitle)/)?.[0] || '';
  const agencyInboxEvidenceRows = js.match(/function renderAgencyInboxEvidenceRows\(evidence = \[\]\)[\s\S]*?(?=\nfunction renderAgencyReceiptRow)/)?.[0] || '';
  assert.match(agencyInboxRow, /ingested →/);
  assert.match(agencyInboxRow, /escapeHtml\(`ingested → \$\{decision\.route \|\| 'unrouted'\}`\)/);
  assert.match(agencyInboxRow, /escapeHtml\(c\.candidateId \|\| 'unknown'\)/);
  assert.match(agencyInboxRow, /c\.receivedAt/);
  assert.match(agencyInboxRow, /escapeHtml\(receivedAt\)/);
  assert.match(agencyInboxRow, /renderAgencyInboxEvidenceRows\(c\.evidence\)/);
  assert.match(agencyInboxEvidenceRows, /escapeHtml\(row\.type \|\| 'unknown'\)/);
  assert.match(agencyInboxEvidenceRows, /escapeHtml\(row\.ref \|\| 'not recorded'\)/);
  assert.match(agencyInboxEvidenceRows, /escapeHtml\(row\.state \|\| 'not recorded'\)/);
  assert.match(agencyInboxEvidenceRows, /escapeHtml\(row\.verifier \|\| 'not recorded'\)/);
  assert.match(agencyReceiptRow, /escapeHtml\(r\.candidateId \|\| 'not linked'\)/);
...
test('Home dashboard exposes a Chat tab plus a glanceable Home preview, not a live tile editor', () => {
  ...
  const chatTranscript = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-chat-transcript.mjs'), 'utf8');
  const standalone = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-chat.html'), 'utf8');

  assert.doesNotMatch(html, /data-home-tile-id="chat"/);
  assert.doesNotMatch(html, /id="chat-overlay"/);
  assert.doesNotMatch(html, /id="chat-slot-overlay"/);
  assert.doesNotMatch(html, /id="chat-slot-tile"/);
  assert.match(html, /id="dashboard-tab-chat"[^>]*data-tab="chat"/);
  assert.match(html, /id="panel-chat"/);
  assert.match(html, /id="chat-slot-tab"/);
  assert.match(html, /id="chat-home-preview"/);
  assert.match(html, /id="chat-preview-snippet"/);
  assert.match(html, /id="chat-shared-template"/);
  assert.match(html, /id="chat-sidebar"/);
  assert.match(html, /id="chat-turn-hud"/);
  assert.match(html, /id="chat-work-strip"/);
  assert.match(html, /id="chat-jump-latest"/);
  assert.match(html, /id="chat-agent-pill"/);
  assert.match(html, /id="chat-model-pill"/);
  assert.match(html, /id="chat-effort-pill"/);
  assert.match(html, /id="chat-thinking-toggle"/);
  assert.doesNotMatch(html, /id="chat-thought-dock"/);
  assert.match(html, /h23-chat-stage/);
  assert.match(html, /h23-chat-stage-primary/);
  assert.match(html, /id="chat-effort-select"/);
  assert.match(html, /data-action="copy-transcript"/);
  ...
  assert.match(standalone, /id="chat-slot-standalone"/);
  assert.match(standalone, /id="chat-shared-template"/);
  assert.match(standalone, /id="chat-effort-pill"/);
  assert.match(standalone, /id="chat-thinking-toggle"/);
  assert.doesNotMatch(standalone, /id="chat-thought-dock"/);
  assert.match(standalone, /data-action="copy-transcript"/);
  assert.match(standalone, /data-action="export-transcript"/);
  assert.match(css, /\.h23-human-card-chat/);
  assert.match(css, /\.h23-human-card-chat[\s\S]*overflow:\s*hidden/);
  assert.match(chatCss, /\.h23-chat-slot\s*>\s*\.h23-chat-shared[\s\S]{0,400}flex-direction:\s*row/);
  assert.match(chatCss, /\.h23-chat-slot\s*>\s*\.h23-chat-shared \.h23-chat-messages[\s\S]{0,500}background:\s*transparent/);
  assert.match(chatCss, /\.h23-chat-slot\s*>\s*\.h23-chat-shared \.h23-chat-messages \.h23-chat-thinking[\s\S]{0,200}display:\s*block/);
  assert.match(css, /\.h23-human-card-chat:not\(\.h23-chat-home-preview\)/);
  assert.match(css, /h23-human-card-chat\.h23-chat-home-preview/);
  assert.match(chatCss, /\.h23-chat-sidebar/);
  assert.match(chatCss, /\.h23-chat-thinking/);
  assert.doesNotMatch(chatCss, /\.h23-chat-thought-dock/);
  assert.match(chatCss, /\.h23-chat-stage/);
  assert.match(chatJs, /thinkingPaused/);
  assert.match(chatJs, /collectThinkingText/);
  assert.match(chatTranscript, /currentTurnOnly/);
  assert.match(chatJs, /appendTurnThinking/);
  assert.match(chatCss, /\.h23-chat-thinking-pill/);
  assert.match(chatCss, /\.h23-chat-tool/);
  assert.match(chatCss, /\.h23-chat-turn-hud/);
  assert.match(chatCss, /\.h23-chat-jump-latest/);
  assert.match(chatCss, /\.h23-chat-work-strip/);
  assert.match(chatJs, /maybeAutoInitDashboardChat/);
  assert.match(chatJs, /setupEffortControls/);
  assert.match(chatJs, /setupThinkingToggle/);
  assert.match(chatJs, /transcript\?\.updateThinking/);
  assert.match(chatJs, /copyTranscript/);
  assert.match(chatJs, /exportTranscript/);
  assert.match(chatJs, /effort: chatReasoningEffort/);
  assert.match(chatJs, /chatMode = mode \|\| 'tab'/);
  assert.match(chatJs, /const shouldResumeHistory = chatMode !== 'preview'/);
  ...
  assert.match(chatJs, /api\/chat\/history\?chatId=/);
  assert.match(chatJs, /api\/work\/stream\?chatId=/);
  assert.match(chatJs, /BroadcastChannel\('home23-chat'\)/);
  assert.match(chatJs, /turn_id: turnId/);
  assert.match(chatJs, /Stopped this reply/);
  assert.match(chatJs, /claimStreamOwnership/);
  assert.match(chatJs, /appendWorkReceipt/);
  assert.match(chatJs, /cursor: activeCursor/);
});
...
test('live work plane can open chat-backed runs, pin machine chats, and steer', () => {
  const html = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-dashboard.html'), 'utf8');
  const chatHtml = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-chat.html'), 'utf8');
  const chatJs = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-chat.js'), 'utf8');
  const chatCss = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-chat.css'), 'utf8');
  const dashJs = fs.readFileSync(path.join(HOME23_ROOT, 'engine/src/dashboard/home23-dashboard.js'), 'utf8');

  assert.match(html, /id="dashboard-tab-work"[^>]*data-tab="work"/);
  assert.match(html, /id="panel-work"/);
  assert.match(html, /id="work-active-list"/);
  assert.match(html, /id="work-recent-list"/);
  assert.match(html, /id="chat-stop-btn"/);
  assert.match(chatHtml, /id="chat-stop-btn"/);

  assert.match(chatJs, /function workChatId\(/);
  assert.match(chatJs, /function openWorkConversation\(/);
  assert.match(chatJs, /function injectSteer\(/);
  assert.match(chatJs, /h23-chat-work-open/);
  assert.match(chatJs, /api\/work\/\$\{encodeURIComponent\(workId\)\}\/inject/);
  assert.match(chatJs, /attachedWorkId && \(chatStreaming \|\| activeTurnId\)/);
  assert.match(chatJs, /pinnedConversationIds/);
  assert.match(chatJs, /startsWith\('subagent:'\)/);
  assert.match(chatJs, /startsWith\('cron-'\)/);
  assert.match(chatJs, /api\/work\/stream\?/);
  assert.doesNotMatch(chatJs, /row\.addEventListener\('click'[\s\S]{0,200}inspectWork/);
  assert.match(chatJs, /searchParams\.get\('work'\)/);
  assert.match(chatCss, /\.h23-chat-work-open/);
  assert.match(chatCss, /\.h23-work-tab/);
  assert.doesNotMatch(dashJs, /os-in-flight[\s\S]{0,80}async-work/);
});
