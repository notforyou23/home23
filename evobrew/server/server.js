...
// Load project-local config.json (generated by Home23) for brain/agent config
// This is the same file that loadConfigurationSync reads via EVOBREW_CONFIG_DIR,
// but we keep a parsed reference for brain/agent-specific sections.
...
try {
  ...
  if (fsSync.existsSync(home23ConfigPath)) {
    ...
    if (home23Config._generated) {
      console.log('[CONFIG] Loaded Home23 evobrew config (config.json)');
    }
  }
} catch (err) {
...
function isAllowedLocalOrigin(origin) {
  const allowed = [
    ...
    /^http:\/\/100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+:\d+$/,  // Tailscale CGNAT
    /^https:\/\/100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+:\d+$/, // Tailscale CGNAT (HTTPS)
    /^https?:\/\/[a-zA-Z0-9-]+\.ts\.net:\d+$/,                          // Tailscale MagicDNS
...
app.get('/api/brain/info', (req, res) => {
  ...
  if (!loader) {
    return res.json({
      ...
      outputsPath: null,
      isAdmin,
      agentBrains: home23Config?.brain?.agentBrains || null
    });
  }
...
// Add Home23 agent brain paths to BRAIN_DIRS so they pass the allowlist check
if (home23Config?.brain?.agentBrains) {
  for (const [agentId, brainPath] of Object.entries(home23Config.brain.agentBrains)) {
    const resolved = path.resolve(brainPath);
    if (!BRAIN_DIRS.some(d => resolved.startsWith(path.resolve(d)))) {
      BRAIN_DIRS.push(resolved);
      console.log(`[Brain] Added Home23 agent brain dir: ${resolved} (${agentId})`);
    }
  }
}
...
} else if (home23Config?.brain?.defaultPath) {
  // Auto-load default brain from Home23 config
  const defaultBrainPath = path.resolve(home23Config.brain.defaultPath);
  const defaultStatePath = path.join(defaultBrainPath, 'state.json.gz');
  if (fsSync.existsSync(defaultStatePath)) {
    loadBrain(defaultBrainPath).then(() => {
      console.log('✅ Default brain auto-loaded from Home23 config\n');
    }).catch(err => {
      console.warn('[Brain] Auto-load default brain failed:', err.message);
    });
  } else {
    console.log('[Brain] Default brain path configured but no state.json.gz found:', defaultBrainPath);
  }
}
